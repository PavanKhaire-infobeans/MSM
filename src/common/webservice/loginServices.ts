...
function loginRequest(url: string, params: Array<any>) {
    const [param] = params
    return Webservice.postRequest(`${url}/api/portals/login`, { checksum: "Q3VlQmFjazIwMTgh" }, param.payload)
}

function loginInstanceRequest(url: string, params: Array<any>) {
    const [param] = params
    return Webservice.postRequest(`${url}/api/portals/authenticate_user`, { checksum: "Q3VlQmFjazIwMTgh" }, param.payload)
}

function getAllIntances(url: string) {
    return Webservice.postRequest(`${url}/api/instances/list`, { checksum: "Q3VlQmFjazIwMTgh" })
}

function getRegistrationFor(url: string) {
    return Webservice.postRequest(`${url}/api/configurations/registration_form_details`, { })
}

function checkPreRegistered(url: string, details: object) {
    return Webservice.postRequest(`${url}/api/alumni/check_pre_registration`, {}, details)
}
...
function logout(url: string, userAuthToken: string) {
    return Webservice.postRequest(`${url}/api/alumni/logout`, { "X-CSRF-TOKEN": userAuthToken }, {}, false)
}

function configurations(url: string, userAuthToken: string) {
    return Webservice.postRequest(`${url}/api/configurations/get`, {"X-CSRF-TOKEN": userAuthToken, "Content-Type" : "application/json"}, { "type": "site"}, false)
}
...
export const SSOLogin = async (params : any) => {
    try {
		let response = await SSOLoginFetch(`https://${CueBackInsatance.InstanceURL}/api/alumni/ssologin`, params)
			.then((response: Response) => response.json())
			.catch((err: Error) => {
                Promise.reject(err)
                EventManager.callBack(kSSOLogin, false, "Unable to process your login request");
			});
		...
		EventManager.callBack(kSSOLogin, false, "Unable to process your request. Please try again later");
    }
