...
export const logoutMethod = () => {	
	...
	LoginStore.logout(Account.selectedData().instanceID, parseInt(Account.selectedData().userID));
...
const WebserviceCall = (function () {
	...
	return {
			...
			return fetch(url, {
				...
				.then((resp: Response) => {
					...
					return rsp
						...
						.then(
							(value: any): any => {
								if (value.ResponseCode == 405 || value.ResponseCode == 402 || value.ResponseCode == 401) {									
									loaderHandler.hideLoader();
		...
		postRequest: (url: string, headers: object, request: object = {}, addRequest: boolean = true): Promise<Response> => {
			let reqMod = {
				...request
			};
			if (addRequest) {
			...
			return fetch(url, {
				...
				.then((resp: any) => {
					...
					return rsp
						...
						.then(
							(value: any): any => {
								console.log("request : received  " + url, new Date().toTimeString());
								if (value.ResponseCode == 405 || value.ResponseCode == 402 || value.ResponseCode == 401) {
									loaderHandler.hideLoader();
									//TODO: Consult Web team and get unique ResponseCode of invalid session.
									if (value.ResponseMessage == "Authentication token expired." || value.ResponseMessage == "Invalid authentication token."
									) {
										ToastMessage('','',true)
										logout();
									}//5461
