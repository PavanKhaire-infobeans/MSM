...
import { View, Image, Animated, Dimensions, FlatList, TouchableOpacity, Keyboard, Platform } from 'react-native';
...
type Props = { actions: Array<ActionSheetItem>, memoryActions? : boolean, title?: any, onActionClick?: (index: number, data? : any) => void, width: string | number, popToAddContent? : boolean }
...
export default class ActionSheet extends React.Component<Props, State> {
    ...
    render() {
        if (this.state.hidden || this.props.actions.length == 0) {
            ...
            return (
                <View style={{ position: 'absolute', backgroundColor: '#00000045', width: '100%', height: '100%', alignItems: 'center', top: 0 }}>
                    <Animated.View style={{ backgroundColor: 'white', maxWidth: 768, width: this.props.width, position: 'absolute', bottom: this.state.bottom, paddingBottom : 15 }}>
                        ...
                        {this.props.title && this.props.title.length > 0 ?                          
                            <TextNew style={{color : "#595959", paddingTop: 15, paddingStart: 24, height: 56, ...fontSize(18), fontWeight: Platform.OS === "ios"? '500':'bold'}}>{this.props.title}</TextNew>:
                        <View></View>
                        }
                        ...
                        <FlatList
                            ...
                            ItemSeparatorComponent={({ leadingItem }: { highlighted: boolean, leadingItem: ActionSheetItem }) => {
                                return (<View style={{ height: 1, backgroundColor: (leadingItem.index == (this.props.actions.length - 2)) ? 'rgba(0.35, 0.35, 0.35, 0.2)' : 'white' }} />)
                            }}
                                ...
                                return (
                                    <TouchableOpacity onPress={() => {
                                        ...
                                        <View style={{ flexDirection: 'row', alignItems: 'center', padding: 10, paddingStart: 24, height: 56 }}>
                                            ...
                                            <TextNew style={{ color: data.isDestructive == 1 ? Colors.NewRadColor : 'black', marginLeft: 24, ...fontSize(18) }}>{data.text}</TextNew>
                                        </View>
                                    </TouchableOpacity>
                                )
                            }}
