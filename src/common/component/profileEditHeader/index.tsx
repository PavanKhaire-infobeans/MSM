...
const styles = EStyleSheet.create({
    titleText: {
        color: '#000', ...fontSize(18), lineHeight: 20, textAlign: 'left',
    ...
    rightButtonsContainer: { flex: 1, paddingTop: 10, paddingRight: 0, flexDirection: 'row', justifyContent: 'flex-end', alignItems: 'center' },
    ...
    _renderMiddle() {
        return (
            <View style={styles.titleContainer}>
                <Text style={styles.titleText} numberOfLines={1} ellipsizeMode='tail'>{this.props.heading}</Text>
            </View>
        );
    }
    ...
    _renderRight() {
        return (
            <View style={[styles.rightButtonsContainer]}>
                <TouchableOpacity onPress={() => this.props.saveValues()}
                    style={styles.rightButtonsTouchable}>
                    <Text style={{ ...fontSize(18), fontWeight: Platform.OS === "ios"? '500':'bold', color: Colors.ThemeColor }}>{this.props.rightText}</Text>
                </TouchableOpacity>
            </View>
        )
    }
