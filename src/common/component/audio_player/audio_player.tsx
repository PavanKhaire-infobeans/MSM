...
export default class AudioPlayer extends React.Component<Props, State> {
  ...
  showPlayer = (index: any) => {
    if (Utility.isInternetConnected) {
      try {
        if (
          this.props.files[index].url == null ||
          this.props.files[index].url == ''
        ) {
        ...
        if (
          this.props.files[index].filesize &&
          this.props.files[index].filesize != '0'
        ) {
          let songTitle = this.props.files[index].file_title
            ? this.props.files[index].file_title
            : this.props.files[index].title
            ? this.props.files[index].title
            : this.props.files[index].filename
            ? this.props.files[index].filename
            : '';
          this.setState({
            playerShowing: true,
          ...
          let path = encode_utf8(
            this.props.files[index].url
              ? this.props.files[index].url
              : this.props.files[index].filePath
              ? this.props.files[index].filePath
              : '',
          );
              ...
              this.player.play(
                () => {
                  this.timeOutBuffering = setTimeout(() => {
                    ...
                    this.currentDuration = setInterval(
                      this.setCurrentTime,
                      500,
                    );
                  }, 2000);
  ...
  setTotalTime = () => {
    if (this.player.duration > 0) {
      this.setState({
        totalDuration: this.setPlayTime(this.player.duration / 1000),
        maximumSeekValue: Math.round(this.player.duration / 1000),
      });
  ...
  setCurrentTime = () => {
    if (
      this.player.currentTime > 0 &&
      (this.state.playing || this.state.slidingInProgess)
    ) {
      // //console.log(this.player._state);
      this.setState({
        currentDuration: this.setPlayTime(this.player.currentTime / 1000),
      });
      if (!this.state.slidingInProgess) {
        this.setState({
          sliderValue: Math.round(this.player.currentTime / 1000),
        });
      }
      if (
        this.player.duration - this.player.currentTime < 500 &&
        this.state.playing
      ) {
    ...
    return (
      <View
        style={{
          ...
          padding: 15,
          position: 'absolute',
          bottom: this.props.bottom ? this.props.bottom : 80,
        }}>
        {this.state.playerShowing && (
          <View
            style={{
              ...
              padding: 20,
              height: this.state.playerShowing
            ...
            {!this.state.minimizedView && (
              <View style={{height: 100}}>
                <Text
                  style={{...fontSize(18), color: '#fff', marginBottom: 5}}
                  numberOfLines={1}>
                  {this.props.memoryTitle}
                </Text>
                <Text
            ...
            <View
              ...
              {this.state.loadingText ? (
                <Text style={{...fontSize(12), color: 'white', width: 80}}>
                  {'Loading...'}
                </Text>
              ) : (
                <Text style={{...fontSize(12), color: 'white', width: 80}}>
...
const PlayPause = (props: {playing: boolean; togglePlayPause: () => void}) => {
  return (
    <TouchableOpacity
      style={{height: 30, width: 30, marginLeft: 25, marginRight: 20}}
  ...
  return (
    <TouchableHighlight
      ...
      <View style={{flexDirection: 'row'}}>
        <PlayButtonDefault
          color={props.disabled ? '#909090' : 'white'}></PlayButtonDefault>
        <PlayButtonDefault
          color={props.disabled ? '#909090' : 'white'}></PlayButtonDefault>
      </View>
    </TouchableHighlight>
...
const BackwardIcon = (props: {disabled: boolean; previousSong: () => void}) => {
  return (
    <TouchableHighlight
      underlayColor={'#00000000'}
      disabled={props.disabled}
      style={{flexDirection: 'row', transform: [{rotate: '180deg'}]}}
