...
import { View, Image, Animated, Dimensions, FlatList, TouchableOpacity, Alert, Keyboard, Platform, StatusBar, DeviceEventEmitter, TouchableHighlight } from 'react-native';
import Text from '../Text';
import DeviceInfo from 'react-native-device-info';
import { fontSize, Colors } from '../../constants';
const { height } = Dimensions.get('screen')
import { checkbox, checkbox_active, action_close, icon_close_black } from "../../../images";
...
type Props = {
    actions: Array<ActionSheetItem>, width?: string, value: any, onItemSelect: (item: ActionSheetItem) => void, selectionType?: any, fullscreen?: boolean, fieldName?: string, label?: string, selectionLimit?: number,
    saveSelectedValues?: (selectedValues: any) => void, selectedValues?: any, maxLimit?: number, 
    title? : string, isFromMultipleDropDown?: boolean, fieldNameOfMultipleDropDown?: string, needHideCallback? : boolean, hideCallBack? : () => void
}

type State = {showError : boolean, errorMessage: any, selectedIndex: any, bottom: any, hidden: boolean, selectedValueObject: any, allSelected: boolean, selectedValues: Array<any>, isMultiSelect: boolean, selectedValueObjects: Object, filteredList: Array<ActionSheetItem>, isSearching: boolean, searchText: string, availableHeight: any}
...
export default class BottomPicker extends React.Component<Props, State> {
    ...
    onItemClickInMultiSelect = (selectedIndex: string, text: string) => {
        ...
        if (index >= 0) {
            ...
            if (this.props.maxLimit > 1 && currentSelectedValues.length >= this.props.maxLimit) {
                this.setState({showError : true, errorMessage :   "You cannot put more than " + this.props.maxLimit + " values in the "+ this.props.label + " field"})
        ...
        this.setStateForSelectedIndex(currentSelectedValues, currentSelectedValueObj);
    }
    ...
    saveCurrentItemValue = (key: string, text: string, disabled: boolean) => {
        if(disabled){
            ...
            if (this.props.isFromMultipleDropDown) {
                this.props.onItemSelect({ key: [this.props.fieldNameOfMultipleDropDown], text })
                this.hidePicker();
            }
        ...
        if (this.state.isSearching) {
            ...
            if (this.state.searchText.trim() == "") {
                ...
                this.props.actions.forEach(element => {
                    let index = element.text.toLocaleLowerCase().indexOf(this.state.searchText.trim().toLocaleLowerCase());
                    if (index >= 0) {
    ...
    render() {
        ...
        if (this.state.hidden || this.props.actions.length == 0) {
            ...
            return (
                <View style={{ position: 'absolute', backgroundColor: 'transparent', bottom: 0, width: '100%', height: '100%', alignItems: 'center' }} onStartShouldSetResponder={() => true} onResponderStart={this.hidePicker}>
      				<StatusBar barStyle={'dark-content'} backgroundColor={Colors.NewThemeColor} />	
                    <Animated.View style={[styles.animatedView, (Platform.OS === "ios"? styles.halfScreenAnimatedView : (this.props.fullscreen ? styles.fullScreenAnimatedView: styles.halfScreenAnimatedView)), {height: this.props.fullscreen ? deviceHeight : 290}]} onStartShouldSetResponder={() => true}>
                        {this.props.fullscreen ?
                            <View>
                                ...
                                {Platform.OS === 'ios' ?
                                    <View style={{ paddingTop: DeviceInfo.hasNotch() ? 30 : 15, borderBottomColor: 'rgba(0, 0, 0, 0.24)', borderBottomWidth: 2 }}>
                                        <ProfileEditHeader showRightText={this.state.isMultiSelect} heading={this.props.label} cancelAction={() => this.hidePicker()} rightText={"Done"} saveValues={() => { this.saveSelectedValues() }} />
                                    </View>
                                    : <View style={{ height: 54, borderBottomColor: 'rgba(0, 0, 0, 0.24)', borderBottomWidth: 2, justifyContent:"center", alignItems: "center" }}>
                                        <ProfileEditHeader showRightText={this.state.isMultiSelect} heading={this.props.label} cancelAction={() => this.hidePicker()} rightText={"Done"} saveValues={() => { this.saveSelectedValues() }} />
                                    </View>}

                                {this.state.showError && <View style={{ width: "100%", backgroundColor: Colors.ErrorColor, justifyContent: 'center', paddingLeft: 10 }}>
                                    <Text numberOfLines={2} style={{ ...fontSize(14), lineHeight: 16, textAlign: 'left', color: '#fff', paddingBottom: 5, paddingTop: 5 }}>
                            ...
                            :
                            <View style={{ borderColor: "#aaaaaa", borderBottomWidth: 1, width: '100%', height: 45, justifyContent: "space-between", flexDirection : "row" }}>
                                <View style={{flex:1, justifyContent: "center", paddingLeft: 16}}>
                                    <Text style={{fontWeight: Platform.OS === "ios"? '600':'bold', ...fontSize(18), color: "#3D3D3D"}}>Choose Your {this.props.title}</Text>
                                </View>
                                <View style={{height: "100%" }}>
                                    <TouchableOpacity onPress={() => { this.hidePicker() }} style={{ flex: 1, justifyContent: 'center', alignItems: 'center', paddingLeft: 11, paddingRight: 16 }}>
                        ...
                        {this.state.isMultiSelect && this.state.filteredList.length > 0 &&this.props.maxLimit <= 1 &&
                            <TouchableOpacity onPress={() => { this.selectAll() }}>
                                <View>
                                    <View style={{ flexDirection: 'row', alignItems: 'center', padding: 10, paddingStart: 5, height: 56 }}>
                                        <Image style={{ marginLeft: 15 }} source={this.state.allSelected ? checkbox_active : checkbox} resizeMode="contain" />
                                        <Text style={{ marginLeft: 15, ...fontSize(16) }}>{"Select All"}</Text>
                                    </View>
                        ...
                        <FlatList
                            ...
                            ItemSeparatorComponent={() => <View style={{ backgroundColor: "#ccc", height: 1, width: "100%", marginRight : 15, marginLeft : 15 }} />}
                            renderItem={({ item: data }: { item: ActionSheetItem }) => {
                                return (
                                    <TouchableHighlight underlayColor={data.disabled ? "transparent" : "#cccccc99"} onPress={() => { this.saveCurrentItemValue(data.key, data.text, (data.disabled? true: false))}}>
                                        <View style={{
                                            flexDirection: 'row', justifyContent: !this.props.fullscreen ? "center" : "flex-start" ,alignItems: 'center', padding: 10, paddingStart: 5, height: 56,
                                            backgroundColor:
                                            (this.props.value == data.key) ||
                                            (!this.state.isMultiSelect &&
                                                (this.state.selectedValues.length > 0 &&
                                                    (this.state.selectedValues[0] == data.key))) ? `${Colors.ThemeColor}23` : '#fff'
                                                }}>
                                            {this.state.isMultiSelect &&
                                                <Image style={{ marginLeft: 15 }} source={this.state.selectedValues.indexOf(data.key) >= 0 ? checkbox_active : checkbox} resizeMode="contain" />}
                                            <Text style={{ marginLeft: 15, ...fontSize(16), color: data.disabled ? "#D3D3D3" : "black" }}>{data.text}</Text>
                                        </View>
                                    </TouchableHighlight>
                                )
                            }}
