...
import { CreateMemoryHelper } from '../../../../../views/createMemory/dataHelper';
    ...
    <Animated.View style={containerStyle}>
      <View style={[style.header, I18nManager.isRTL && style.reverseHeader]}>
        ...
        <TouchableOpacity
          activeOpacity={0.7}
          style={style.arrowWrapper}
          onPress={() => !prevDisable && onSelectYear(year == new Date().getFullYear() ? +0 : +1)}>
          <Image
            resizeMode='contain'
            source={require('../../assets/arrow.png')}
      ...
      {
         showYears && newMemoryYears.length ?
            <FlatList
              data={newMemoryYears}
              numColumns={3}
              scrollEnabled={true}
              renderItem={(item) => {
                ...
                return (
                  <TouchableOpacity
                    key={item.text}
                    activeOpacity={0.8}
                    style={[style.item, selectYear === item.text && style.selectedItem]}
                    onPress={() => {
                      setSelectYear(item.text);
                      setShowYears(false);
                      onChangeYear(item.text)
                    }}>
                    <Text
                      style={[
                        style.itemText,
                        selectYear === item.text && style.selectedItemText,
                        //  style.disabledItemText,
                      ]}>
                      {item.text}
                    </Text>
                  </TouchableOpacity>
      ...
      {!showYears &&<View style={[style.monthList, utils.flexDirection]}>
