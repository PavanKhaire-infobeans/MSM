...
export default class MultipleDropDownSelector extends React.Component<Props, State> {
	...
	render() {
		return (
			<View style={[this.props.style, { justifyContent: "flex-start", flexDirection: 'column' }]}>

				<View style={{ minWidth: 180, flexDirection: 'row', marginBottom: 5 }}>
					<Text style={{ lineHeight: 26, fontSize: 18, color: "#6B6B6B" }}>{this.props.placeholderText}</Text>
					{this.props.isRequired ? <Text style={{ color: Colors.NewRadColor }}>{" *"}</Text> : null}
				</View>

				<View style={{ height: 56, flexDirection: "row", justifyContent: "space-between", marginTop: 10 }}>
					<DropDownSelector value={[this.props.view1Value]} placeholderText={this.props.view1Title} selectedValue={this.props.view1Value} onOptionSelected={() => this.props.onOptionSelected(this.props.view1Title)}></DropDownSelector>
					<DropDownSelector value={[this.props.view2Value]} placeholderText={this.props.view2Title} selectedValue={this.props.view2Value} onOptionSelected={() => this.props.onOptionSelected(this.props.view2Title)}></DropDownSelector>
				</View>
