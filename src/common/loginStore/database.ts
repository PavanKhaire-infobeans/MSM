    ...
    function checkDB(db: any) {
        return new Promise((resolve: Function) => {
            db.transaction((tx: any) => {
                tx.executeSql("CREATE TABLE IF NOT EXISTS accounts_table " +
                    ...
                    "instanceImage VARCHAR(100), isSSOLogin BOOLEAN, is_public_site BOOLEAN, PRIMARY KEY (instanceID, userID));", [], (_: any, results: any) => {
                        if (results) {
            ...
            return new Promise((resolve: Function, reject: Function) => {
                openDB((db: any) => {
                    if (db) {
                        db.transaction((tx: any) => {
                            if (tx) {
                                let query = "INSERT INTO accounts_table (instanceID, email, " +
                                    ...
                                    `${loginDetails.userID}, '${getSanitizedString(loginDetails.firstName)}', ` +
            ...
            return new Promise((resolve: Function, reject: Function) => {
                openDB((db: any) => {
                    if (db) {
                        db.transaction((tx: any) => {
                            if (tx) {
                                let query = "UPDATE accounts_table SET userAuthToken = ''" +
            ...
            return new Promise((resolve: Function, reject: Function) => {
                openDB((db: any) => {
                    if (db) {
                        db.transaction((tx: any) => {
                            if (tx) {
                                let query = "UPDATE accounts_table SET " +
                                    "email = " + `'${loginDetails.email}', ` +
                                    "userAuthToken = " + `'${loginDetails.userAuthToken}', ` +
                                    "name = " + `'${loginDetails.name}', ` +
                                    "instanceURL = " + `'${loginDetails.instanceURL}', ` +
                                    "firstName = " + `'${getSanitizedString(loginDetails.firstName)}', ` +
                                    "lastName = " + `'${getSanitizedString(loginDetails.lastName)}', ` +
                                    "profileImage = " + `'${loginDetails.profileImage}', ` +
                                    "instanceImage = " + `'${loginDetails.instanceImage}', ` +
                                    "is_public_site = " + `'${loginDetails.is_public_site}',` +
                                    "isSSOLogin = " + `'${loginDetails.isSSOLogin}'` + 
                                    " WHERE instanceID=" + `${loginDetails.instanceID}`+
                                    " AND userID=" + `${loginDetails.userID}`;
            ...
            return new Promise((resolve: Function, reject: Function) => {
                openDB((db: any) => {
                    if (db) {
                        db.transaction((tx: any) => {
                            if (tx) {
                                tx.executeSql("SELECT * FROM accounts_table order by email", [], (_: any, results: any) => {
                                    // console.log("Query completed", results);
                                    if (results) {
                                        resolve(results);
                                    } else {
            ...
            new Promise((resolve: Function, reject: Function) => {
                openDB((db: any) => {
                    if (db) {
                        db.transaction((tx: any) => {
                            if (tx) {
                                tx.executeSql("SELECT * FROM accounts_table WHERE instanceID=" + instanceID, [], (_: any, results: any) => {
                                    //console.log("Query completed", results);
                                    if (results) {
                                        resolve(results);
                                    } else {
