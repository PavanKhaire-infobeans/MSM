...
const MindPopStore = (() => {
    ...
    function checkDB(db: any) {
        return new Promise((resolve: Function) => {
            db.transaction((tx: any) => {
                ...
                var queryCreateMindPopAttachment = `CREATE TABLE IF NOT EXISTS ${Table.Names.MindPopAttachments} (${
                ...
                console.log("Query to create MindPop Attachment Table:", queryCreateMindPopAttachment);
                ...
                tx.executeSql(queryCreateMindPop, [], (_: any, results: any) => {
                    ...
                    if (results) {
                        ...
                        tx.executeSql(queryCreateMindPopAttachment, [], (_: any, results: any) => {
                            console.log("Mindpop attachment create table Query completed", results);
                            if (results) {
                                resolve(results);
                            } else {
                                resolve(false);
    ...
    async function openDB(onOpened: (db: any) => void) {
        ...
        var db = SQLite.openDatabase(dbName, "1.0", "Cueback Database", 200000, openCB, errorCB);
        await checkDB(db);
        onOpened(db);
    }
    ...
    function _getAttachmentEntities(attachementArray: any[], fileType: number, mindPopID: number, instanceID: number): MindPopAttachment[] {
    ...
    function _createDBEntities(dataObj: { [x: string]: any }): MindPop[] {
        let itemsWebservice: any[] = getValue(dataObj, ["Details", "mindPopList"]) || [];
        ...
        if (itemsWebservice.length > 0) {
            mindpopItems = itemsWebservice.map(obj => {
                ...
                let imageAttachments = _getAttachmentEntities(images, FileType.image, mindPopID, instanceID);
                let audioAttachments = _getAttachmentEntities(audios, FileType.audio, mindPopID, instanceID);
                let videoAttachments = _getAttachmentEntities(videos, FileType.video, mindPopID, instanceID);
                let fileAttachments = _getAttachmentEntities(files, FileType.file, mindPopID, instanceID);
    ...
    function prepareInsertionStatement(attachments: MindPopAttachment[]): string[] {
        var arrayOfAttachmentValue: string[] = [];
        for (var i = 0; i < attachments.length; i++) {
            let file = attachments[i];
            var values = `(${file.instanceID}, ${file.mindpopID}, '${file.id}', '${file.fileName}', ${file.type},'${file.fileMime}', '${file.url}', '${
                file.thumbnailURL}', '${file.title}', '${Account.selectedData().userID}')`;
    ...
    function getAttachmentInsertionStatement(mindPop: MindPop): string[] {
        ...
        statementString = arrayOfAttachmentValue.length > 0 ? arrayOfAttachmentValue.join(",") : "";
        ...
        return new Promise((resolve: Function, reject: Function) => {
            openDB((db: any) => {
                if (db) {
                    db.transaction((tx: any) => {
                        if (tx) {
                            var query = `REPLACE INTO ${Table.Names.MindPopAttachments} VALUES` + " ";
    ...
    const _deleteMindPopAttachment = (ids?: number[], mindPopIDs?: number[]) =>{
        ...
        if (ids != null && ids.length !== 0) {
            query = `DELETE FROM ${Table.Names.MindPopAttachments} WHERE ${Table.MindPopAttachment.id} IN (${ids.join(",")}) AND ${
                Table.MindPopAttachment.instanceID
                } = ${instanceID} AND ${Table.MindPopAttachment.userId} = ${Account.selectedData().userID}`;
        } else if (mindPopIDs != null && mindPopIDs.length != 0) {
            query = `DELETE FROM ${Table.Names.MindPopAttachments} WHERE ${Table.MindPopAttachment.mindPopID} IN (${mindPopIDs.join(",")}) AND ${
                Table.MindPopAttachment.instanceID
                } = ${instanceID}  AND ${Table.MindPopAttachment.userId} = ${Account.selectedData().userID}`;
        } else {
            query = `DELETE FROM ${Table.Names.MindPopAttachments}  WHERE ${Table.MindPopAttachment.instanceID} = ${instanceID}  AND ${Table.MindPopAttachment.userId} = ${Account.selectedData().userID}`;
        }
    ...
    return {
                                ...
                                if (tx) {
                                 ...
                                 ${Table.MindPop.lastModified} LONG, PRIMARY KEY (${Table.MindPop.instanceID}, ${Table.MindPop.id})
                                    ...
                                    for (var i = 0; i < mindPopDBEntities.length; i++) {
                                        ...
                                        var values = `(${obj.instanceID}, ${obj.id}, "${obj.message}", ${obj.lastModified}, ${Account.selectedData().userID})`
                                        arrayOfValues.push(values)
                                        let attachments: string[] = getAttachmentInsertionStatement(obj)
                                    ...
                                    tx.executeSql(query, [], (_: any, results: any) => {
                                        ...
                                        if (results) {
                                            if (queryAttachment.trim().length != 0) {
                                                saveMindPopAttachments(queryAttachment).then((result) => {
                                                    resolve(result)
                                                }, (error) => {
        ...
        addMindPop: (itemList: Array<{
            instanceID: number, id: string,
            message: string, lastModified: string
        }>) => {
            var query: string = `INSERT INTO ${Table.Names.MindPop} VALUES` + " "
            var arrayOfValues = []
            for (let obj of itemList) {
                arrayOfValues.push(`(${obj.instanceID}, ${obj.id}, '${obj.message}', ${obj.lastModified}, ${Account.selectedData().userID})`)
            }
            let valueString = arrayOfValues.join(',')
            query = query + valueString
            console.log("Insert Query: ", query);
            ...
            return new Promise((resolve: Function, reject: Function) => {
                openDB((db: any) => {
                    if (db) {
                        db.transaction((tx: any) => {
                            if (tx) {
                                let query = `SELECT mp.instanceID, mp.id, mp.message, mp.lastModified, mpa.id as attachmentID, mpa.fileName, mpa.url, mpa.type, mpa.fileMime, mpa.thumbnailURL from ${Table.Names.MindPop} mp LEFT JOIN ${Table.Names.MindPopAttachments} mpa on mp.id = mpa.mindPopID WHERE mp.message  like '%${searchKeyword}%' and mp.instanceID = ${Account.selectedData().instanceID}  and mp.userId = ${Account.selectedData().userID} ORDER BY mp.id DESC, mpa.id DESC`
        ...
        _deleteMindPops: (ids: number[] = null) => {
            ...
            let instanceID = Account.selectedData().instanceID
            if (ids == null || ids.length == 0) {
                query = `DELETE FROM ${Table.Names.MindPop} WHERE ${Table.MindPop.instanceID} = ${instanceID} AND ${Table.MindPop.userId} = ${Account.selectedData().userID}`
            } else {
                query = `DELETE FROM ${Table.Names.MindPop} WHERE ${Table.MindPop.id} IN (${ids.join(',')}) AND ${Table.MindPop.instanceID} = ${instanceID} AND ${Table.MindPop.userId} = ${Account.selectedData().userID}`
            }
        ...
        _getMindPopAttachments: (id: string) => {
            let instanceID = Account.selectedData().instanceID
            let query: string = `SELECT * FROM ${Table.Names.MindPopAttachments} WHERE ${Table.MindPopAttachment.mindPopID} = "${id}" AND ${Table.MindPopAttachment.instanceID} = ${instanceID} AND ${Table.MindPop.userId} = ${Account.selectedData().userID}`
