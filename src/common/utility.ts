...
export default class Utility {
    ...
    static notificationObject :any = {hasNotification : false, data : {}, isBackgroundNotification : false};
            ...
            if (completeDate) {
                try {
                    ...
                    if (date_format == "M d, Y") {
                        let formattedDate = shortMonths[parseInt(month) - 1] + " " +
                        date + ", " + year;
                        ...
                        let formattedDate = months[parseInt(month) - 1] + " " +
                        date + ", " + year;
                        return formattedDate;
    ...
    static dateObjectToDefaultFormat(completeDate: Date) {
        let formattedDate = completeDate.getUTCFullYear() + "-" +
            ((completeDate.getUTCMonth() + 1) > 9 ? (completeDate.getUTCMonth() + 1) : ("0" + (completeDate.getUTCMonth() + 1))) + "-" +
            (completeDate.getUTCDate() > 9 ? completeDate.getUTCDate() : ("0" + completeDate.getUTCDate())) + " " +
            (completeDate.getUTCHours() > 9 ? completeDate.getUTCHours() : ("0" + completeDate.getUTCHours())) + ":" +
            (completeDate.getUTCMinutes() > 9 ? completeDate.getUTCMinutes() : ("0" + completeDate.getUTCMinutes())) + ":" +
            (completeDate.getUTCSeconds() > 9 ? completeDate.getUTCSeconds() : ("0" + completeDate.getUTCSeconds()));
        return formattedDate;
    }
    ...
    static timeDuration(createdOn : any, formatt :any, onlyAgoTime? : boolean){
        try{
            let createdDate = Utility.dateObjectToDefaultFormat(new Date(parseInt(createdOn) * 1000));
            let addString = ""
            if (formatt == "M d, Y, t"){
                createdDate = Utility.dateObjectToDateAndTimeFormat(new Date(parseInt(createdOn) * 1000));
            } else {
                addString = "on "
            }
            ...
            return addString+Utility.dateAccordingToFormat(""+createdDate, formatt);
        } catch (execption){
            return 0;
        }
    }
    static  getActionAlertTitle= (action: any): any => {
        ...
        switch(action){
            ...
            case MemoryActionKeys.removeMeFromThisPostKey:
                ...
                alertData.message = "Are you sure you want to remove your self from this post?";
                break;
            case MemoryActionKeys.blockAndReportKey:
                alertData.title = "Block user and Report memory";
                alertData.message = "Are you sure you want to block user and report this post?";
                break;
