...
import { BackHandler, Platform, Text, View, Alert, DeviceEventEmitter } from "react-native";
import DeviceInfo from "react-native-device-info";
import EStyleSheet from "react-native-extended-stylesheet";
import { ActionConst, Actions, Drawer, Router, Scene, Stack, Tabs } from "react-native-router-flux";
...
import { EditHeader, iPadList, MindPopEdit, MindPopList, PreviewImage } from "./views/mindPop";
...
import { kForegroundNotice, kBackgroundNotice } from "./views/notificationView/notificationServices";
...
const AppRouter = () => (
    <Router>
        <Scene key="root">
            ...
            <Scene key="splash" type={ActionConst.RESET} hideNavBar component={Splash} />
            <Scene key="prologue" type={ActionConst.RESET} hideNavBar component={Prologue} />
            <Scene key="appIntro" type={ActionConst.RESET} hideNavBar component={AppIntro} />
            <Scene key="findCommunity" type={ActionConst.PUSH} hideNavBar component={FindCommunity} />
            <Scene key="registrationPre" type={ActionConst.PUSH} hideNavBar component={RegFirstStep} />
            <Scene key="registrationFinal" type={ActionConst.PUSH} hideNavBar component={RegFinalStep} />
            <Scene key="userRegStatus" type={ActionConst.PUSH} hideNavBar component={UserRegistrationStatus} />
            <Scene key="login" type={ActionConst.PUSH} hideNavBar component={LoginView} />
            <Scene key="commonInstanceListsSelection" type={ActionConst.PUSH} hideNavBar component={CommonInstanceListsSelection} />
            <Scene key="forgotPassword" type={ActionConst.PUSH} hideNavBar component={ForgotPassword} />
            <Scene key="filtersScreen" type={ActionConst.PUSH} hideNavBar component={FilterScreen} />
            <Drawer key="dashBoard" type={ActionConst.RESET} hideNavBar contentComponent={Menu}>
                <Scene key="wrapper" hideNavBar>
                    <Tabs
                        ...
                        tabBarOnPress={({ navigation, defaultHandler }) => navigateToParticular(navigation, defaultHandler)}
                        tabBarStyle={{ height: 57, justifyContent: "center", alignItems: "center", paddingBottom: 0, overflow: "hidden" }}>
                        ...
                        <Stack title={TabItems.MyMemories} tabBarIcon={TabIcon}>
                            <Scene hideNavBar key="memoriesDrafts" title={TabItems.MyMemories} component={MyMemoriesContainer} />
                        </Stack>
                        <Stack title={TabItems.AddContent} tabBarIcon={TabIcon} >
                            <Scene hideNavBar key="contentView" title={TabItems.AddContent} component={Dashboard} />
                        </Stack>
                        <Stack title={TabItems.Prompts} tabBarIcon={TabIcon}>
                            <Scene hideNavBar key="promptsView" title={TabItems.Prompts} component={PromptsView} />
                        </Stack>
            ...
            <Scene key="addContent" type={ActionConst.PUSH} hideNavBar component={AddContentView} />
            <Scene key="userProfileEdit" type={ActionConst.PUSH} hideNavBar component={UserProfileEdit} />
            <Scene key="multipleValuesEdit" type={ActionConst.PUSH} hideNavBar component={MultipleValuesEdit} />
            <Scene key="profile" type={ActionConst.PUSH} hideNavBar component={Profile} />
            <Scene key="myAccount" type={ActionConst.PUSH} title={"My Account"} hideNavBar component={MyAccount} />
            <Scene key="createMemory" type={ActionConst.PUSH} title={"Memory Draft"} hideNavBar component={CreateMemory} />
            <Scene key="memoryDetails" hideNavBar component={MemoryDetails} />
            <Scene key="customListMemoryDetails" hideNavBar component={CustomListView} />
            ...
            <Scene key="commonAudioRecorder" hideNavBar component={CommonAudioRecorder} />
            <Scene key="fileDescription" hideNavBar component={FileDescription} />
            <Scene key="publishMemoryDraft" hideNavBar component={PublishMemoryDraft} />
            <Scene key="commonListCreateMemory" hideNavBar component={CommonListCreateMemory} />
            <Scene key="collectionList" hideNavBar component={CollectionList} />
            <Scene key="memoryCollectionList" hideNavBar component={MemoryCollectionList} />
            <Scene key="collectionDetails" hideNavBar component={CollectionDetails} />
            <Scene key="createRenameCollection" hideNavBar component={CreateRenameCollection} />
            <Scene key="whoCanSee" hideNavBar component={WhoCanSee} />
            <Scene key="inviteCollaborators" hideNavBar component={InviteCollaborators} />
            <Scene key="notesToCollaborators" hideNavBar component={NotesToCollaborators} />
            <Scene key="commonFriendsSearchView" hideNavBar component={CommonFriendsSearchView} />
            <Scene key="etherPadEditing" hideNavBar component={EtherPadEditing} />
            <Scene key="notificationListing" hideNavBar component={NotificationListing} />
            <Scene key="changePassword" type={ActionConst.PUSH} hideNavBar={true} component={ChangePassword} />
            <Scene key="commonWebView" hideNavBar component={CommonWebView} />
            <Scene key="notificationView" title={"Notifications"} hideNavBar component={NotificationView} />
...
class App extends React.Component {
    ...
    async componentDidMount() {
        ...
        if (Platform.OS == "android") {
            this.backEvent = BackHandler.addEventListener("hardwareBackPress", this._backPressAnd);
        }
    }
            ...
            firebase.messaging().requestPermission()
                .then(() => {
                    firebase.messaging().getToken().then(fcmToken => {
                        if (fcmToken) {
                            DefaultPreference.set('firebaseToken', fcmToken).then(function () { });
                            return true;
                        }
                    });
                })
    ...
    componentWillUnmount() {
        ...
        this.backEvent && this.backEvent.removeListener() && this.onTokenRefreshListener();
    }
