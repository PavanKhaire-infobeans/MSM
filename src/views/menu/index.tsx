...
import { View, TouchableOpacity, FlatList, Alert, Image, DeviceEventEmitter, EmitterSubscription, SafeAreaView } from "react-native";
import Text from "../../common/component/Text";
import { Actions } from "react-native-router-flux";
import { Storage, Size, ERROR_MESSAGE, Colors, fontSize } from "../../common/constants";
...
class Menu extends React.Component<MenuProps> {
	...
	componentDidMount() {
		...
		LoginStore.listAllAccounts()
			.then((resp: any) => {
				...
				this.setState({ list: list.filter((it: UserData) => it.userAuthToken != "") });
			})
			.catch((err: Error) => {
				//console.log(err);
		});
		...
		return (
			<SafeAreaView style={{ flex: 1 }}>
				<View
					...
					<Text style={{ color: "black", ...fontSize(16) }}>Your communities</Text>
					{false ? (
						<TouchableOpacity style={{ height: 45, paddingLeft: 10, paddingRight: 10, justifyContent: "center", alignItems: "center" }}>
							<Text style={{ color: Colors.NewTitleColor, ...fontSize(16) }}>Edit</Text>
						</TouchableOpacity>
					) : null}
				</View>
				<FlatList
	...
	_logout = () => {
		LoginStore.listAllAccounts()
			.then((resp: any) => {
				...
				if(list.length > 1){
					Actions.push("commonInstanceListsSelection", {listAccounts : list, title: 'Logout', type : 'logout',  onClick : logoutWorkFlow})
				} else{
					Alert.alert(
						"",
	...
	drawHeader=(data: any)=>{
		if(data.item.type != "AddCommunity" && this.userEmail != data.item.email) {
			...
			return <View style={{width: "100%", padding: 15, paddingTop : data.index == 0 ? 0 : 15, paddingBottom: 7, paddingLeft : 5}}>
						...
						<View style={{flexDirection : "row", alignItems: "center"}}>
							...
							<Text style={{paddingLeft: 10, ...fontSize(14), color: "rgba(61, 61, 61, 0.8)"}}>{data.item.email}</Text>
						</View>
					</View>
		}
		return null;
	...
	renderRow = (data: { item: UserData & { type: string }; index: number }) => {		
		...
		if (this.props.user.instanceID == data.item.instanceID && this.props.user.userID == data.item.userID) {
			return (				
				<View>
					{this.drawHeader(data)}
					<Banner
						communityInfo={data.item}
...
const logoutWorkFlow = (selectedAccounts : any) => {
	logoutMultiple(selectedAccounts).then((resp: any) => {
			let list = resp.rows.raw();
			var accounts = list.filter((it: UserData) => it.userAuthToken != "");			
			if (accounts.length > 0) {
				EventManager.callBack(kOnLogout, accounts);
