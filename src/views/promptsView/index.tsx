...
import { SafeAreaView, View, TouchableOpacity, Image, FlatList, Platform, StatusBar, Text, Alert, Keyboard, ActivityIndicator, RefreshControl, TouchableHighlight } from "react-native";
...
import { close_big, edit_icon, edit_prompt, filter_icon, topics_filter } from "../../images";
...
import { GetPrompts, kPromptsList, kHidePrompt, HidePrompt } from "../myMemories/myMemoriesWebService";
 ...
 export default class PromptsView extends React.Component<State,Props> {
    ...
    constructor(props: Props) {
        super(props);  
        this.scrollFlatlistListener = EventManager.addListener("scrollFlatlist",()=>{
            this.flatListRef.scrollToOffset({ offset: 0, animated: false });
        })
        this.promptsListListener = EventManager.addListener(kPromptsList, (fetched? : boolean,ifLoadMore? : boolean,fetchPromptsList?: any)=>{
            if(fetched){ 
         ...
         this.promptHideListener = EventManager.addListener(kHidePrompt, (fetched? : boolean, promptId? : any)=>{
            if(fetched){ 
              ...
              <SafeAreaView style={{width: "100%",flex: 1, backgroundColor : "#fff"}}>    
                ...
                <NavigationBar title={TabItems.Prompts}/>
                  <StatusBar barStyle={'dark-content'} backgroundColor={Colors.NewThemeColor} />
                   { !this.state.loading && <View style={{borderTopColor: "#fff",borderTopWidth : 0.5, opacity : 0.75}}>  
                      <TouchableOpacity onPress={()=> Actions.push("topicsFilter",{categories : this.state.categoriesArray}) } underlayColor={'transparent'} style={{height: 40, borderRadius : 200,margin : 10,borderWidth: 1, borderColor:"#373852",backgroundColor : "#373852", paddingRight: 16, paddingLeft : 16, alignItems: "center", alignSelf : "flex-end"}}>
                        <View style={{flexDirection : 'row', paddingTop : Platform.OS == "ios" ? 8 : 6, justifyContent: "space-between", alignItems: "center"}}>
                            <TextNew style={{fontWeight : '400', color : "#fff", ...fontSize(18), marginRight : 5}}>Topics</TextNew>
                    ...
                    <FlatList
                        ...
                        renderItem={(item: any) => {
                                return (
                                    <View style={{margin: 10,width : "90%", borderRadius : 15, backgroundColor : "#FFFFFD", height : 200, borderWidth : 0, justifyContent: "space-between",alignSelf : "center", shadowOpacity: 0.2,elevation : 0.2}}>
                                         <View> 
                                            <TouchableOpacity underlayColor={'transparent'} onPress={()=> this.hidePrompt(item.item.id)} style={{alignItems : "flex-end", margin : 11, paddingRight: 2}}> 
                                                <Image source={close_big}></Image>
                                            </TouchableOpacity>
                                          </View>
                                           <View style={{flex:1}}><TextNew numberOfLines={3} style={{ ...fontSize(21),color: "#373852", fontWeight: Platform.OS === "ios"? '500':'bold', marginLeft: 20, flex: 1,marginRight: 20, marginBottom: 16, textAlign: "center" ,paddingTop: 15}}>{item.item.desc}</TextNew></View>
                                            <View>
                                                <TouchableOpacity onPress={()=> this.convertToMemory(item.item.id, item.item.desc)}>
                                                    <View style= {{flexDirection : "row", backgroundColor : "#4EB79E",padding:16,justifyContent: "center", borderBottomLeftRadius: 15, borderBottomRightRadius : 15}}>
                                                        <Image source={edit_prompt}></Image>
                                                        <TextNew style={{ ...fontSize(18), marginLeft: 5,fontWeight: '400', color: "#FDFDFD"}}>Add your Memory</TextNew>
                                                    </View>
                                                </TouchableOpacity>
                                            </View>
                                    </View>
                        ...
                        { this.state.items.length == 0 && 
                         <View style={{height : '100%', width: '100%', alignContent: 'center', justifyContent: 'center', backgroundColor: 'white'}}>
                           {this.state.loading ?<ActivityIndicator color={Colors.ThemeColor}    
                                                                size="large"                    
                                                                style={{flex: 1, justifyContent : "center"}}/> :  <Text style={{...fontSize(16), color : Colors.dullText, textAlign: 'center'}}>There are no prompts to display at this moment</Text> }
                          </View> }
                 </View>
              </SafeAreaView>
              </View>
    ...
    hidePrompt(promptId : any){
        Alert.alert(
            "Hide Prompt",
            `Are you sure you want to hide this prompt?`,
            [
