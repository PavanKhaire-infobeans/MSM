...
import { View, TouchableOpacity, KeyboardAvoidingView, SafeAreaView, TextInput, StatusBar, Keyboard } from "react-native";
...
import { ForgotPasswordState, ForgotPasswordServiceStatus } from "./forgotPasswordReducer";
import { connect } from "react-redux";

import TextField from "../../common/component/textField";
import { Size, testEmail, getValue, Colors, fontSize, Storage } from "../../common/constants";
...
class ForgotPassword extends React.Component<Props> {
	...
	componentWillReceiveProps(nextProps: Props) {
		if (nextProps.forgotPasswordStatus.completed) {
			...
			if (nextProps.forgotPasswordStatus.success) {
				let message: string = getValue(nextProps, ["forgotPasswordStatus", "data", "ResponseMessage"]);
				let code: number = getValue(nextProps, ["forgotPasswordStatus", "data", "ResponseCode"]);
	...
	render() {
		return (
			<SafeAreaView style={styles.container}>
				...
				<NavigationHeaderSafeArea heading={"Forgot Password"} showCommunity={false} cancelAction={()=> Actions.pop()} 
					...
					<KeyboardAwareScrollView
						...
						<View style={[styles.loginContainer, { justifyContent: "space-between", width: '100%', marginTop: Size.byHeight(40) }]}>
							{/** Commuity banner UI */}
							<View style={{ width: Size.byWidth(310), justifyContent: "flex-start", alignItems: "flex-start" }}>
								<View style={{ width: "100%" }}>
									{/* <View style={styles.communityBanner}>
										<CommunityBanner communityInfo={this.selectedCommunity} />
									</View> */}

									<Text style={{paddingBottom: 15, textAlign: "center", fontWeight: "300", ...fontSize(Size.byWidth(18)) , color: Colors.TextColor}}>
								...
								<View style={{ width: "100%" }}>
									{this.state.isRequestSubmitted ? (
										<View style={{ flex: 1, height: 100 }}>
											<SubmitButton text="Done" onPress={this.onDoneButtonAction.bind(this)} />
										</View>
									) : (
											<KeyboardAvoidingView style={styles.keyboardAvoiding} behavior="padding">
												...
												<SubmitButton text="Reset Password" onPress={this.resetButtonAction.bind(this)} />
							...
							<View style={{ width: "100%", alignItems: "center" }}>
								{this.state.isRequestSubmitted && (
									<View style={{ paddingBottom: 40 }}>
										<Text style={{ paddingTop: 0, textAlign: "center", fontWeight: "400", ...fontSize(Size.byWidth(16)) }}>
											Didn't receive the Email?
										</Text>
										<Text
											style={{ color: "#595959", paddingTop: 25, textAlign: "center", fontWeight: "300", ...fontSize(Size.byWidth(16)) }}>
											Please check your spam folder, or
										</Text>
										<TouchableOpacity style={{ paddingTop: 3, paddingBottom: 40 }} onPress={this.resetButtonAction.bind(this)}>
											<Text style={{ color: Colors.NewTitleColor, textAlign: "center", fontWeight: "400", ...fontSize(Size.byWidth(16)) }}>
	...
	resetButtonAction() {
		...
		this.FetchLoginInstance([{"payload":{"emailId": this.state.email, "details": {"type":  "public" }}}]);
	...
	callForgetPasswordService=(instances: any)=>{
		loaderHandler.showLoader();
		this.props.forgotPasswordServiceCall({ "emailId": this.state.email, "instances": instances})
	}

	FetchLoginInstance = async (params: any) => {
		try {		
			loaderHandler.showLoader();
			let response = await loginInstanceRequest(`${kAdmin}`, params).then((response: Response) => 
												response.json()).
												catch((err: any) => console.log(err))			
			if(response.ResponseCode==200){
				if(response.Response.length > 1){
					Actions.push("commonInstanceListsSelection", {listAccounts : response.Response, title: 'Forgot Password', type : ListType.ForgotPassword, email: this.state.email.trim(),  onClick : this.callForgetPasswordService})
					loaderHandler.hideLoader()
				} else {
...
const mapDispatch = (dispatch: Function) => ({
	forgotPasswordServiceCall: (params: object) => dispatch({ type: ForgotPasswordServiceStatus.RequestStarted, payload: params }),
