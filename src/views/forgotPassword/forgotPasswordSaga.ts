...
function* fetchRequest(params: any) {
    return forgotPasswordRequest(kAdmin, params).then((response: Response) => response.json())
}
...
function* ForgotPasswordService(...action: any[]) {
    try {
        ...
        yield put({ type: ForgotPasswordServiceStatus.RequestSuccess, payload: responseBody })
    } catch (err) {
...
export function* ForgotPasswordServiceCall() {
    yield takeLatest(ForgotPasswordServiceStatus.RequestStarted, ForgotPasswordService)
} 