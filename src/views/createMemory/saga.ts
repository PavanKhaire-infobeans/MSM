...
import { LocationListUpdated, RecentTags, SaveSearchList, CollectionList, SaveCollaborators, SaveDescription } from "./reducer";
...
function* getListLocations(params: any) {
	delay(500);
	return MemoryService(`https://${Account.selectedData().instanceURL}/api/location/get`, params)
...
function* getMemoryTagsList(params: any) {
	delay(500);
	return MemoryService(`https://${Account.selectedData().instanceURL}/api/get/memory_tags`, params)
		.then((response: Response) => response.json())
		.catch((err: Error) => Promise.reject(err));
}
...
function* getUserList(params: any) {
	delay(500);
	return MemoryService(`https://${Account.selectedData().instanceURL}/api/get/users`, params)
		.then((response: Response) => response.json())
		.catch((err: Error) => Promise.reject(err));
}
...
function* getCollectionList(params: any) {
	delay(500);
	return MemoryService(`https://${Account.selectedData().instanceURL}/api/collection/list`, params)
		.then((response: Response) => response.json())
		.catch((err: Error) => Promise.reject(err));
}
...
function* getCollaboratorsList(params: any) {
	delay(500);
	return MemoryService(`https://${Account.selectedData().instanceURL}/api/collaborator/list`, params)
		.then((response: Response) => response.json())
		.catch((err: Error) => Promise.reject(err));
}

function* etherpadEditing(params: any) {	
	return MemoryService(`https://${Account.selectedData().instanceURL}/api/etherpad/get_set_text`, params)
		.then((response: Response) => response.json())
		.catch((err: Error) => Promise.reject(err));
}
	...
	try {
		...
		if(searchType==kSearchTags){
			...
			if(value && value.length > 0){
				...
				yield put({ type : SaveSearchList , payload : [{tid: TimeStampMilliSeconds(), name : requestData.payload.searchTerm}]})
			}
		} else{
...
function* getUsers(requestData: any) {	
	try {
		...
		if(value.length>0){
			...
			yield put({ type : SaveSearchList , payload : [{name : "We couldn't find any Friend Circle that matched "+requestData.payload.searchTerm+". Try searching for another Friend Circle.", field_first_name_value : "We couldn't find anyone that matched "+requestData.payload.searchTerm+". Try searching for someone else.", field_last_name_value:"", uid : -1, id : -1}]})				
		}
	} catch (err) {
        //console.log(err);
	}
...
export function* watchCreateMemories() {
	yield all([takeLatest(LocationAPI, getLocationList), 
