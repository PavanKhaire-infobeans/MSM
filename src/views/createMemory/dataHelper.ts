        ...
        if(fieldName == "year"){
            ...
            if(parseInt(year) == new Date().getFullYear()){
                    ...
                    for(let i = 0; i < MonthObj.month.length ; i++){
                            if(i>MonthObj.serverMonthsCount-1){
                                    let index = i;
                                    (index-MonthObj.serverMonthsCount) <= currentMonth ? actions.push({key: MonthObj.month[i].tid, text: MonthObj.month[i].name}) : actions.push({key: MonthObj.month[i].tid, text: MonthObj.month[i].name, disabled: true})    

                            } else{
                                    actions.push({key: MonthObj.month[i].tid, text: MonthObj.month[i].name})    
                            }
                    }
            }
            else{
            ...
            if(MonthObj.selectedIndex > MonthObj.serverMonthsCount-1){
                    switch(MonthObj.month[MonthObj.selectedIndex].name){
                            ...
                            break;
                            case "Apr" || "Jun" || "Sep" || "Nov" : limit = 30;
                                    break;
                            default :     
                    }
            }
            if(year == new Date().getFullYear() && MonthObj.selectedIndex == new Date().getMonth()+MonthObj.serverMonthsCount){
...
export const DefaultCreateMemoryObj=(key : any, initialState : any, isOwner : boolean)=>{   
        let details : any = {};
        ...
        if(isOwner){
                ...
                if(initialState.collections && initialState.collections.length && initialState.collections.length > 0){
                ...
                if(initialState.tags){
                        details = {...details, memory_tags_text: getCommaSeparatedArray("name", initialState.tags)}
                }
                if(initialState.whoElseWhereThere){
                        details = {...details, who_else_was_there_uids: getCommaSeparatedArray("uid", initialState.whoElseWhereThere)}
                }	
                if(initialState.shareOption == "custom"){
                        if(initialState.whoCanSeeMemoryUids){
                                details = {...details, who_can_see_memory_uids: getCommaSeparatedArray("uid", initialState.whoCanSeeMemoryUids)}
                        }	
                        if(initialState.whoCanSeeMemoryGroupIds){
                                details = {...details, who_can_see_memory_group_ids: getCommaSeparatedArray("id", initialState.whoCanSeeMemoryGroupIds)}
                        }	
                }
...
export const getEtherPadUrl=(padId : any)=>{
    let padUrl  = "https://collaboration.cueback.com/p/" + padId + 
