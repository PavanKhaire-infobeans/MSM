import { View, SafeAreaView, StatusBar, ScrollView, Image, TouchableOpacity, Text, TextInput, KeyboardAvoidingView, Platform, Keyboard, TouchableHighlight, FlatList } from "react-native";
import React from 'react';
import { Actions } from "react-native-router-flux";
// @ts-ignore
import { KeyboardAwareScrollView } from "../../common/component/keyboardaware-scrollview";
import { Colors, fontSize, decode_utf8, getValue } from "../../../common/constants";
...
class CreateRenameCollection extends React.Component<Props, State> {     
    ...
    componentDidMount(){
        ...
        this.collectionUpdated = EventManager.addListener(kCollectionUpdated, this.collectionUpdate)
    }
    ...
    saveValue=()=>{
        if(this.state.content.trim().length > 0){
            ...
            if(this.props.isRename){
                UpdateMemoryCollection({"collection_tid" : this.props.tid, "name" : decode_utf8(this.state.content.trim())}, false)
            } else {
                UpdateMemoryCollection({"name" : decode_utf8(this.state.content.trim())}, false)
            }
        } else{
    ...
    return (
        ...
        <SafeAreaView style={{width: "100%", flex: 1, backgroundColor : "#fff"}}> 
           <View style={{flex: 1}}>
               <NavigationHeaderSafeArea heading={this.props.isRename ? "Rename Collection " :"Create New Collection"} cancelAction={() => this.cancelAction()} showRightText={true} rightText={"Done"} saveValues={this.saveValue}/>
               {/* <SafeAreaView style={{width: "100%", flex: 1, backgroundColor : "#fff"}}>                    */}
               ...
               <View style={{justifyContent:"flex-start",alignItems : "flex-start", padding: 15}}>
                    <TextInput
						...
						style={{
                            ...
                            borderBottomColor: this.state.showError ? Colors.ErrorColor : "rgba(0,0,0,0.4)",
                            borderBottomWidth : 1
						}}/>
                        {this.state.showError && <Text style={{...fontSize(12), color : Colors.ErrorColor}}>*Please enter collection name</Text> }
               </View>
               </View>
               </SafeAreaView>
           </View>
