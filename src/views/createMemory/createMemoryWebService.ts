import { Storage, asyncGen, getValue, uploadTask, TimeStampMilliSeconds} from "../../common/constants";
...
export const CreateUpdateMemory = async (params: any, filesToUpload : any, listener: any, key?: any) => {
	try {
		console.log("params..",params);
		let data = await Storage.get('userData')
		let response = await MemoryService(`https://${Account.selectedData().instanceURL}/api/mystory/create_update`,
...
export const GetDraftsDetails = async (nid : any) => {
	try {
		let data = await Storage.get('userData')
		let response = await MemoryService(`https://${Account.selectedData().instanceURL}/api/mystory/edit_memory_values`,
			[{ "X-CSRF-TOKEN": data.userAuthToken, "Content-Type": "application/json" },
			...
			if (response != undefined && response != null){
				if(response.ResponseCode==200){					
					...
					EventManager.callBack(kDraftDetailsFetched, false, response["ResponseMessage"]);
				}		
			}	
	} catch (err) {
		EventManager.callBack(kDraftDetailsFetched, false, "Unable to process your request. Please try again later");
	}
};
...
export const GetCollectionDetails= async (memoryId: any)=>{
    try {
		let data = await Storage.get('userData')
		let response = await MemoryService(`https://${Account.selectedData().instanceURL}/api/collection/list_collection_memories`,
			[{ "X-CSRF-TOKEN": data.userAuthToken, "Content-Type": "application/json" },
			 {"details" : {                
		...
		EventManager.callBack(kCollectionMemories, false, "Unable to get memory details");                            
	}
};

export const UpdateAttachments= async (nid: any, fileDetails : any, key: any)=>{  
    try {
		let data = await Storage.get('userData')
		let response = await MemoryService(`https://${Account.selectedData().instanceURL}/api/mystory/edit_delete_file`,
			[{ "X-CSRF-TOKEN": data.userAuthToken, "Content-Type": "application/json" },
...
export const UpdateMemoryCollection= async (params: any , isSequenceUpdated : any)=>{
    ...
    try {
		...
		let data = await Storage.get('userData')
		let response = await MemoryService(`https://${Account.selectedData().instanceURL}/api/collection/add_edit`,
			[{ "X-CSRF-TOKEN": data.userAuthToken, "Content-Type": "application/json" },
			 {"details" : params}
			])
			.then((response: Response) => response.json())
...
export const InviteCollaborators= async (nid: any, friendsList : any, friendCircle : any, note : "")=>{
    try {
		let data = await Storage.get('userData')
		let response = await MemoryService(`https://${Account.selectedData().instanceURL}/api/collaborator/invite`,
			[{ "X-CSRF-TOKEN": data.userAuthToken, "Content-Type": "application/json" },
			 {"details" :  { nid: nid,
...
export const DeleteDraftService= async (id: any, action: any, listener: any)=>{
    try {        
		let data = await Storage.get('userData')
...
export const CollaboratorActionAPI= async (params: any)=>{
    try {        
		let data = await Storage.get('userData')
		let response = await MemoryService(`https://${Account.selectedData().instanceURL}/api/collaborator/actions`,
			[{ "X-CSRF-TOKEN": data.userAuthToken, "Content-Type": "application/json" },
			 {  "configurationTimestamp" : "0",
                 "details" : params
            }])
...
async function uploadFile(memoryId: number, file: TempFile) {
    ...
    if (memoryId) {
        options["parameters"] = { nid: `${memoryId}`, 
								  file_title : `${file.file_title}`,
		                          file_description : `${file.file_description}` };
    }

	if(getValue(file, ["filename"])){
		options["parameters"] = {...options["parameters"], title : getValue(file, ["filename"])}
	}
    return new Promise((resolve, reject) => {
        uploadTask(
            (data: any) => {
