...
import { View, TouchableOpacity, SafeAreaView, TextInput, TouchableWithoutFeedback, Keyboard, Animated, StatusBar } from "react-native";
...
import {ChangePasswordService,kChangePassword} from '../changePassword/changePasswordWebService';
...
export default class ChangePassword extends React.Component {
	...
	constructor(props: any) {
		super(props);		
		this.changePasswordListener = EventManager.addListener(kChangePassword, this.changePasswordResponse);            
...
/**
    ...
    onTextChange(key: string, value: string) {
        ...
        if (key == "currentPassword" && this.state.currentPasswordError.error == true && value.length > 0) {
            state = {
        ...
        if (key == "newPassword" && this.state.newPasswordError.error == true && value.length > 0) {
            state = {
		...
		if (key == "confirmPassword" && this.state.confirmPasswordError.error == true && value.length > 0) {
            state = {
	...
	onClick() {
		...
		if(newPassword.trim().length > 5){
			...
			if(!(rgularExp.containsNumber.test(newPassword.trim()) &&
	...
	render() {
		return (
			<SafeAreaView style={styles.container}>	
				...
				<NavigationHeaderSafeArea ref={(ref)=> this.navBar = ref}heading={"Change Password"} showCommunity={false} cancelAction={this.cancelAction} 
                                      showRightText={false} isWhite={true}/>
				...
				<KeyboardAwareScrollView 
					...
					<SubmitButton text="Save Password" onPress={this.onClick.bind(this)} />								
				</KeyboardAwareScrollView>
