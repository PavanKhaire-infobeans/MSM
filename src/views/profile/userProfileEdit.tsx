...
import { SafeAreaView, View, TouchableOpacity, Image, TextInput, Dimensions, ActivityIndicator, Modal, Alert, Keyboard } from "react-native";
...
export default class UserProfileEdit extends React.Component<Props> {
	...
	saveProfileData() {
		...
		if(Utility.isInternetConnected){
			...
			No_Internet_Warning('Your changes may not be saved as you are not connected to the Internet. Please check your connection and try again.');
		}
	}
		...
		for (let keys in editablefields) {
			...
			if (!(hasExistingValue || isUpdated) && isRequired) {
				if (type == "options_select" || type == "options" || type == "options_buttons" || type == "date_select") {
					...
					error = {
						...error, [currentField.field_name]: { error: true, message: `Please enter ${currentField.label}` }
					};
				}
			}
		}
	...
	render() {
		...
		return (
			<View style={{ flex: 1 , backgroundColor: "#fff"}}>
				<NavigationBarForEdit rightText="Save" saveValues={() => this.saveProfileData()} heading={"Basic Info"} cancelAction={() => this._closeAction()}></NavigationBarForEdit>
				<SafeAreaView style={{ flex: 1, justifyContent: 'center', alignItems: "center", backgroundColor: "#fff" }}>
					<KeyboardAwareScrollView
						...
						<View style={{ width: DeviceInfo.isTablet() ? 320 : "90%", justifyContent: "center", alignItems: "center", paddingTop: 40, marginBottom: 20 }}>
							...
							<TouchableOpacity style={{ height: 33, width: 200, margin: 20, backgroundColor: Colors.NewYellowColor, borderRadius: 50, justifyContent: "center", alignItems: "center" }}
								onPress={() => {
									this.setState({
										actionSheet: {
						...
						<View style={{ width: DeviceInfo.isTablet() ? 320 : "90%", justifyContent: "center" }}>
