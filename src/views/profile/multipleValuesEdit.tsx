...
import { SafeAreaView, View, TouchableOpacity, Image, TextInput, Text, Keyboard, StatusBar } from "react-native";
...
import { requestPermission, Colors, GenerateRandomID, getValue, testEmail, testPhone, NO_INTERNET, fontSize } from "../../common/constants";
import ImageCropPicker, { Image as PickerImage } from "react-native-image-crop-picker";
import { Actions } from "react-native-router-flux";
import { UpdateFormValues, UploadProfilePic, PhotoType, RemoveProfilePic, UserProfile } from "./userProfileWebService";
...
type Props = { sectionHeading: any, editableFields: any, basicInfo?: any, profilePicUri?: any }
type State = {
    actionSheet: {
    ...
    hasLoaded: boolean;       
    [key: string]: any | string; error: { [x: string]: { error: boolean; message: string } };
...
export default class MutilpleValueEdit extends React.Component<Props> {
    ...
    constructor(props: Props) {
        ...
        if(props.basicInfo){
            ...
            this.profileUpdated = EventManager.addListener(kSetUserProfileData, () => { Keyboard.dismiss(); Actions.popTo("profile") });
    ...
    onOptionSelection(field: any) {
        ...
        if (field.type == "date" || field.type == "date_select") {
            if (getValue(field, ["granularity", "todate"]) != "required") {
                this.setState({ isDatePickerVisible: true, selectionData: { fieldName: field.field_name } })
                return
            }
        ...
        this.setState({
            selectionData: {
                ...
                label: field.label,
                selectedValues: this.state[field.field_name] ? this.state[field.field_name] : field.default_value,
    ...
    onOptionSelectForMultipleDropDown = (field_name: string, default_value_from: string,
        default_value_to: string, view1Title: string, view2Title: string, selectedViewName: string) => {
        var actions: ActionSheetItem[] = [];

        let today = new Date().getFullYear()
        let least = 1917
        let most = today
        let defaultFrom = default_value_from != "" ? parseInt(default_value_from) : least;
        ...
        this.setState({
            selectionData: {
                ...
                fieldNameOfMultipleDropDown: selectedViewName == view1Title ? "value" : "value2"
    ...
    getValueForField(field: any): any {
        ...
        if (type == "options_select" || type == "options" || type == "options_buttons") {
            ...
            if (getValue(field, ["granularity", "todate"]) == "required") {
                let default_value_from = this.state[field.field_name] ? (this.state[field.field_name].value ? this.state[field.field_name].value : (field.default_value.value)) : (field.default_value.value);
                let default_value_to = this.state[field.field_name] ? (this.state[field.field_name].value2 ? this.state[field.field_name].value2 : (field.default_value.value2)) : (field.default_value.value2)
    ...
    generateSectionFields() {
        ...
        return <View >
            {this.props.editableFields.map((field: any, index: number) => {
                ...
                if (type == "options_select" || type == "options" || type == "options_buttons") {
                    let valueArray: string[] = []
                    let default_values = this.state[field.field_name] ? this.state[field.field_name] : field.default_value
                    for (let key in default_values) {
                        //console.log("Values while selection ", default_values[key])
                        valueArray.push(default_values[key]);
                    }
                    ...
                    return <DropDown key={`${index}`} placeholderText={field.label} value={default_value}
                    ...
                    // default_value = this.getValueForField(field)                    
                    let keyboardBoardType = field.field_name && (field.field_name).indexOf("phone") >= 0 ? "phone-pad" : ((field.field_name).indexOf("email") >= 0 ? "email-address" : "ascii-capable")
                    ...
                    return <TextField
                        ...
                        onChange={(text: string) => {
                            this.setState({ [field.field_name]: text, error: { ...this.state.error, [field.field_name]: { error: false, message: "" } } });
                        }}
                        {...extra}
                        returnKeyType={this.lastFieldName === field.field_name ? "done" : "next"}
                        value={this.state[field.field_name] ?
                            this.state[field.field_name] : this.state[field.field_name] == "" ? "" : default_value}
                    ...
                    if (getValue(field, ["granularity", "todate"]) == "required") {
                        ...
                        default_value_from = Utility.dateAccordingToFormat(default_value_from, "Y") ? Utility.dateAccordingToFormat(default_value_from, "Y") : ""
                        default_value_to = Utility.dateAccordingToFormat(default_value_to, "Y") ? Utility.dateAccordingToFormat(default_value_to, "Y") : ""
                        return <MultipleDropDownSelector placeholderText={field.label}
                            {...extra}
                            view1Value={default_value_from}
                            view2Value={default_value_to}
                            view1Title="From" view2Title="To"
                            onOptionSelected={(selectedFrom: string) => { this.onOptionSelectForMultipleDropDown(field.field_name, default_value_from, default_value_to, "From", "To", selectedFrom) }}
                        />
                    } else {
                        let field_date: any = this.state[field.field_name] ? this.state[field.field_name] : field.default_value.value;
                        field_date = field_date.length > 0 ? Utility.dateAccordingToFormat(field_date, field.granularity.date_format) : ""
                        return <DropDown key={field.field_name} selectedValue={field_date} value={field_date} placeholderText={field.label} onOptionSelected={() => { this.onOptionSelection(field) }} />;
                    }
                }
            })}
        </View>
    ...
    generateProfilePicView=()=>{
		...
		let imageURL = (stateValue && stateValue != "") ? stateValue : getValue(this.props, ["profilePicUri"])
		imageURL = imageURL || "";
		let profilePicURL = imageURL != "" ? Utility.getFileURLFromPublicURL(imageURL) : ""
		let isProfilePicAvailable = this.isProfilePicAvailable
       return <View style={{ width: DeviceInfo.isTablet() ? 320 : "90%", justifyContent: "center", alignItems: "center", paddingTop: 40, marginBottom: 20 }}>
							<View style={{ justifyContent: 'center', alignItems: 'center' }}>
								<Image
									defaultSource={profile_placeholder}
									source={isProfilePicAvailable ? { uri: profilePicURL } : profile_placeholder}
									style={{ width: 100, height: 100, borderRadius: 50 }}
									onLoad={() => this.setState({ hasLoaded: true })}
									onLoadStart={() => this.setState({ hasLoaded: false })}
								/>
								{!this.state.hasLoaded ? <ActivityIndicatorView size="small" /> : null}
							</View>


							<TouchableOpacity style={{ height: 33, width: 200, margin: 20, backgroundColor: Colors.ThemeColor, borderRadius: 50, justifyContent: "center", alignItems: "center" }}
								onPress={() => {
									this.setState({
										actionSheet: {
							...
							>
								<TextNew style={{ lineHeight: 20,...fontSize(16), textAlign: "center", color: "#fff" }}>{"Change Profile Picture"}</TextNew>
							</TouchableOpacity>

							{profilePicURL != "" ? <TouchableOpacity style={{ justifyContent: "center", alignItems: "center" }}
								onPress={() => { this.removeImage() }}
							>
								<TextNew style={{ lineHeight: 20, ...fontSize(16), textAlign: "center", color: Colors.NewRadColor }}>{"Remove Photo"}</TextNew>
    ...
    validateFields = (): boolean => {
        ...
        for (let keys in editablefields) {
            ...
            let hasExistingValue = existingValue != null && existingValue != "" && typeof existingValue != "undefined"
            ...
            if (currentField.type == "date_select") {
                if (getValue(currentField, ["granularity", "todate"]) == "required") {
                    ...
                    hasExistingValue = default_value_from.length > 0 && default_value_to.length > 0
                }
            ...
            if ((currentField.field_name).indexOf("email") != -1 && isUpdated) {
                if (!testEmail(updatedValue)) {
                    error = {
                        ...error, [currentField.field_name]: { error: true, message: "Please enter valid email address" }
                    };
                }
            }
            ...
            if ((currentField.field_name).indexOf("phone") != -1 && isUpdated) {
                if (!testPhone(updatedValue)) {
                    error = {
                        ...error, [currentField.field_name]: { error: true, message: "Please enter valid phone number " }
                    };
                }
            }
            ...
            if((currentField.field_name).indexOf("name") != -1 && isRequired){  
                ...
                let value = this.state[fieldName] != null && this.state[fieldName] != undefined ? this.state[fieldName] : existingValue;        
                if(value==null || value==undefined || value.trim().length == 0){
                    error = {
                        ...error, [currentField.field_name]: { error: true, message: "Please enter text" }
                    };
                }              
            }
            ...
            if (!(hasExistingValue || isUpdated) && isRequired) {
                if (type == "options_select" || type == "options" || type == "options_buttons" || type == "date_select") {
                    error = { ...error, [currentField.field_name]: { error: true, message: `Please select ${currentField.label}` } };
                } else {
                    error = {
                        ...error, [currentField.field_name]: { error: true, message: "Please enter text" }
                    };
	...
	uploadImage = (imageFile: TempFile) => {
		...
		UploadProfilePic(imageFile).then((response: any) => {
	...
	removeImage = () => {
		...
		RemoveProfilePic().then((response: any) => {
            loaderHandler.hideLoader()
    ...
    render() {
        return (
            <View style={{ flex: 1 }}>
                ...
                <NavigationHeaderSafeArea isWhite={true} rightText="Save" saveValues={() => this.saveProfileData()} heading={this.props.sectionHeading} cancelAction={() => {Keyboard.dismiss(); Actions.pop()}}/>
                    <KeyboardAwareScrollView
                        ...
                        <View style={{ width: DeviceInfo.isTablet() ? 320 : "90%", paddingTop: 20, marginBottom: 0 }}>
                            {this.generateSectionFields()}
                            <DateTimePicker isVisible={this.state.isDatePickerVisible} onCancel={() => {
                ...
                <BottomPicker
                    ...
                    onItemSelect={(selectedItem: ActionSheetItem) => {
                        ...
                        if (this.state.selectionData.isFromMultipleDropDown) {
                            ...
                            this.setState({ [fieldName]: { ...this.state[fieldName], [selectedItem.key]: Utility.dateObjectToDefaultFormat(date) }, error: { ...this.state.error, [fieldName]: { error: false, message: "" } } }, () => {
                                //console.log("Data setting completed", this.state)
                            });
                        } else {
                            this.setState({ [fieldName]: { [selectedItem.key]: selectedItem.text }, error: { ...this.state.error, [fieldName]: { error: false, message: "" } } });
                        }
                    }}
                    ...
                    isFromMultipleDropDown={this.state.selectionData.isFromMultipleDropDown}
                    fieldNameOfMultipleDropDown={this.state.selectionData.fieldNameOfMultipleDropDown}
                    saveSelectedValues={(selectedValueObjects: any) => {
                        let fieldName = this.state.selectionData.fieldName;
                        this.setState({ [fieldName]: selectedValueObjects, error: { ...this.state.error, [fieldName]: { error: false, message: "" } } });
                        //console.log(this.state)
                    }}
                    />
            </View>
