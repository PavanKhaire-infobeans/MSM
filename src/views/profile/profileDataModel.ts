...
export interface FieldStruct {
    label: string;
    type: "sub" | "sub-single" | "date_select" | "options_select" | "text_textfield";
...
export class ProfileDataModel {
                ...
                for (let keys in subSections) {
                    ...
                    if (allKeys.length > 0) {
                        ...
                        if (key == "group_basic_info") {
                            switch (currentKey) {
                                case "group_user_contact_info": this.contactInfoSection = formSection;
                                    this.updateContactInfo(fields)
                                    break;
                                case "group_user_basic_info": this.basicInfoSection = formSection;
            ...
            if (currentField.field_name == "field_profile_picture") {
                ...
                if (currentValue && typeof currentValue != "undefined") {
                    for (let ky in currentField.current_value) {
                        this.basicInfo.profilePicUri = currentField.current_value[ky].thumbnail_preview;
                        Account.selectedData().profileImage = Utility.getFileURLFromPublicURL(currentField.current_value[ky].thumbnail90);
    ...
    updateAllFormSections(heading: string, fields: []) {
        ...
        for (let keys in fields) {
            ...
            if (currentField.hasOwnProperty("date_settings")) {
                let granularityObj = getValue(currentField.date_settings, ["granularity"])
                if (granularityObj) {
                    //console.log("DOB Granulaity before", granularityObj);
                    let toDateRequired = getValue(currentField.date_settings, ["todate"]) || ""
                    let dateFormat = getValue(currentField.date_settings, ["date_format"]) || ""
                    granularity = { ...granularityObj, "todate": toDateRequired, "date_format": dateFormat };
                }
            ...
            if (type == "options_select" || type == "options" || type == "options_buttons") {
                ...
                default_value = getValue(this.valueForObjectArray(getValue(currentField, ["current_value"])), ["value"])
            }
            else if (type == "date_select") {
                let valueObject = this.valueForObjectArray(getValue(currentField, ["current_value"]));
                let value = getValue(valueObject, ["value"]) ? getValue(valueObject, ["value"]) : "";
                let value2 = getValue(valueObject, ["value2"]) ? getValue(valueObject, ["value2"]) : "";
    ...
    updateBasicInfo(fieldsForBasicInfo: any) {
        for (let keys in fieldsForBasicInfo) {
            ...
            if (fetchedValue) {
                let val: any = getValue(this.valueForObjectArray(fetchedValue), ["value"])
                if (val) {
                    value = val;
                }
            }
            switch (currentObject.field_name) {
                ...
                case "field_birthday":
                    ...
                    let date_value = Utility.dateAccordingToFormat(value, currentObject.date_settings.date_format);//getValue(this.valueForObjectArray(getValue(currentObject, ["current_value"])), ["value"])                    
    ...
    updateContactInfo(fieldsForContactInfo: any) {
        ...
        let completeAddressValues = [
            this.contactInfo.address_line_1, this.contactInfo.address_line_2, this.contactInfo.city,
            this.contactInfo.state, this.contactInfo.country, this.contactInfo.zipcode
        ]
