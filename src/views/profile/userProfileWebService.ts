import { userProfile, removeProfilePicture } from "../../common/webservice/userProfile";
import EventManager from "../../common/eventManager";
import { Storage, getValue, uploadTask, ERROR_MESSAGE } from "../../common/constants";
...
export const UserProfile = async () => {
	...
	try {
		...
		let response = await userProfile(`https://${Account.selectedData().instanceURL}/api/alumni/profile_page_details`,
...
export const UpdateFormValues = (state: any, editableFields: any) => {
	...
	for (let keys in editableFields) {
		...
		let value2: any = "";
		let value1: any = "";
		if ((state[currentField.field_name] != null && state[currentField.field_name] != undefined) &&
		currentField.default_value != state[currentField.field_name]) {
			...
			switch (currentField.type) {
				...
				case "date_select":
				   ...
				   let startValue = value1 ? value1 : currentField["default_value"].value;
			...
			if (value2 && value2.trim().length > 0) {
				updatedValues = {
					...
					[currentField.field_name]: {
						...
						"value": (value1 && value1.trim().length > 0) ? value1 :currentField["default_value"].value,
						"value2": value2
					}
				}
			}
...
const updateUserProfile = async (updatedFieldsData: Object) => {
	...
	try {
		let data = await Storage.get('userData')
		//console.log(data);
		let response = await userProfile(`https://${Account.selectedData().instanceURL}/api/alumni/update`,
			[{ "X-CSRF-TOKEN": data.userAuthToken, "Content-Type": "application/json" },
			{
...
export const UploadProfilePic = async function uploadProfilePicture(file: TempFile, type?: number) {
	...
	let options: { [x: string]: any } = {
		url: `https://${Account.selectedData().instanceURL}/api/alumni/profile_picture_upload`,
		path: filePath,
		method: "POST",
...
export const RemoveProfilePic = async function RemoveProfilePicture(type?: number) {
	...
	return new Promise((resolve, reject) => {
		...
		removeProfilePicture(`https://${Account.selectedData().instanceURL}/api/alumni/profile_picture_upload`,
