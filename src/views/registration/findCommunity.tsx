...
type Item = { InstanceID: string; InstanceName: string; InstanceURL: string; InstanceImageURL: string };

type State = { isSearching: boolean; searchText: string; selectedItem?: Item };
type FindProps = { request: { completed: boolean }; end: Function; getAllInstances: Function; list: Item[], openLoginDrawer? : () => void };
		...
		if(!item.is_fake) {
			...
			if(Utility.isInternetConnected){											
				Account.tempData().values = {
					...
					instanceURL: item.InstanceURL.replace("https://", "").replace("http://", "") //'192.168.2.6'
				};
		...
		return (
			<SafeAreaView style={{ flex: 1, alignItems: "center", backgroundColor: "#fff" }}>
				...
				{/* <TouchableHighlight onPress={() => Keyboard.dismiss()} underlayColor="#fff" style={{ width: "100%", flex: 1, alignItems: "center" }}> */}
					<View style={{ flex: 1, padding: 16, paddingTop: 10, alignItems: "center", backgroundColor: "transparent"}}>						
						<Text style={{...fontSize(15), color: "#777777", paddingBottom: 10}}>Search for your private community by organization or URL</Text>
						<SearchBar
						...
						/>
						<View style={{ flex: 1, padding: 16, paddingTop: 24, width: this.screenSize.width, marginBottom: 60 + (Platform.OS == "ios" && StaticSafeAreaInsets.safeAreaInsetsBottom ? StaticSafeAreaInsets.safeAreaInsetsBottom + 10: 0)}}>
...
class ListItems extends React.Component<{ list: Item[]; onSelect: (item: Item) => void }> {
	_renderItem = ({ item: rowData, index }: ListRenderItemInfo<Item>): React.ReactElement<any> => {
		let style = {};
		return (
			<TouchableHighlight
				underlayColor={"#fff"}
				...
				<InstanceView
					...
					communityInfo={{
						...
						instanceURL: rowData.InstanceURL.replace("https://", "").replace("http://", ""),
						instanceImage: rowData.InstanceImageURL || "https://admin.cueback.com/sites/default/files/gradcap_icon.png"
					}}
				/>
			</TouchableHighlight>
		);
	...
	render() {
		return (
			<View style={{ flex: 1, height: "100%", width: "100%" }} onMoveShouldSetResponder={() => false} onTouchStart={()=> Keyboard.dismiss()}>
				<Text style={{...fontSize(15), color : "#000"}}>Select the community you want to log into or send a request to join</Text>
				<FlatList					
					bounces={false}					
