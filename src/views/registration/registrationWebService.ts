import { getRegistrationFor, registrationSubmit, checkPreRegistered } from "../../common/webservice/loginServices";
...
export interface FormStruct {
	label: string;
	type: "sub" | "sub-single" | "date_select" | "options_select" | "text_textfield" | "options_buttons";
	form?: FormStruct[];
...
export const registrationForm = async () => {
	try {
		if(Utility.isInternetConnected){
			let response: Response = await getRegistrationFor(`https://${Account.tempData().instanceURL}`);
			let resp: { ResponseCode: number; ResponseMessage: string; Details: { form1: object } } = await (async () => response.json())();
...
export const checkUserRegistration = async (submitData: any) => {
	try {
		if(Utility.isInternetConnected){
			...
			let chkResponse: Response = await checkPreRegistered(`https://${Account.tempData().instanceURL}`, submitData);
			let resp: { ResponseCode: number; ResponseMessage: string; isRegistered: number, personalInfo?: any } = await (async () => chkResponse.json())();
			loaderHandler.hideLoader();
			EventManager.callBack(kCheckUserProfile, true, resp.isRegistered == 1, resp.personalInfo);
		} else{
			No_Internet_Warning()
		}	
	} catch (err) {
...
export const submitRegistration = async (registrationData: any) => {
	try {
		if(Utility.isInternetConnected){
			...
			let resp: { ResponseCode: number; ResponseMessage: string; userId: number } = await registrationSubmit(`https://${Account.tempData().instanceURL}`, registrationData).then((response: Response) => {
			...
			loaderHandler.hideLoader();
			if (resp.ResponseCode == 200) {
				EventManager.callBack(kSubmitFormItem, true, resp.ResponseMessage, registrationData);
			} else {
...
function toForm(items: { [x: string]: any }): FormStruct[] {
	...
	for (let key in items) {
		...
		if (typeof wSettings == "object" && !Array.isArray(wSettings) && Object.keys(wSettings).length > 1) {
			let keys = [];
			for (let ky in settings) {
				if (ky.indexOf("default_value") == 0 && ky.indexOf("default_value_") == -1 && settings[ky] == "blank") {
					keys.push(ky);
				}
			}
				...
				form:
					currentObject.widget.module == "date"
						? keys.map((ky: string, index: number) => ({
							...
							...(typeof currentObject["#multiple"] == "undefined" ? { multiple: currentObject["#multiple"] } : {}),
							required: currentObject.required ? true : false,
