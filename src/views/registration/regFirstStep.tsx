...
export default class RegFirstStep extends Component<Props> {
	...
	constructor(props: Props) {
		...
		console.log("registrationFormData this.props.: ", JSON.stringify(this.props.formList))
			...
			if (isRegistered && role == "Alumni") {
				if (this.props.isCuebackRegistration) {
					if (this.props.navBar) {
						...
						this.props.navBar.onRegFinalCallBack(`An account with the email already exists, please login to continue`);
					}
					else {
						Alert.alert(isRegistered)
	...
	submitRegisterResponse = (success: boolean, message: string, registrationData: any, formError: any) => {
		if (success) {
			if (this.props.isCuebackRegistration) {
				if (this.props.navBar) {
					Alert.alert(message)
					this.props.navBar.onRegFinalCallBack(`User registered successfully. Please verify link on email and login to continue`);
				}
				else {
					Alert.alert(message)
				...
				<TextField
					...
					onSubmitEditing={() => {
						...
						this.setState({
							...
							this.regScroll.scrollTo({ x: 0, y: this.state.scrollHeight, animated: true })
						})
					}}
				/>
			</View>
		...
		return (
			<View style={{ height: Dimensions.get('window').height - 100, paddingHorizontal: 24, paddingBottom: 20 + (Platform.OS == "ios" && StaticSafeAreaInsets.safeAreaInsetsBottom ? StaticSafeAreaInsets.safeAreaInsetsBottom + 50 : 0) }}>
				...
				>
					...
					<View style={{ height: this.state.regFirstStep ? (130 - this.state.keyboardHeight) : (300 - this.state.keyboardHeight) }} />
					<TouchableWithoutFeedback onPress={() => {
											...
											for (let fitm of frmItem.form) {
												if (typeof sbForm[sKey] == "undefined" || (typeof sbForm[sKey] == "object" && Object.keys(sbForm[sKey]).length == 0)) {
													...
													if (!getValue(sbForm, [sKey, keyVal])) {
														error = { ...error, [fitm.field_name]: { error: true, message: `Please enter ${fitm.label}${fitm.module == "date" ? " year" : ""}` } };
													}
													else if (validBirthYear(sbForm[sKey][keyVal])) {
														error = { ...error, [fitm.field_name]: { error: true, message: `Please enter valid ${fitm.label}${fitm.module == "date" ? " year" : ""}` } };
													}
												}
											}
						...
						disabled={this.state.regFirstStep ? (this.state["field_first_name"] && (this.state["field_last_name"]) && (this.state["emailAddress"]) && (this.state["default_value"])) ? false : true : (this.state["password"] && (this.state["repeat_password"])) ? false : true}
					>
	...
	render() {
		...
		const yVal = this.moveOnYAxis.interpolate({
			inputRange: [0, 0.5, 1],
			outputRange: [0, -(this.state.keyboardHeight * 1.2), -(this.state.keyboardHeight * 1.2)],
		});
