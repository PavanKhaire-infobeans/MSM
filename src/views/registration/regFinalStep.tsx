			...
			<StatusBar barStyle={'dark-content'} backgroundColor={Colors.NewThemeColor} />
				<KeyboardAwareScrollView
					...
					<View style={{ width: 310, paddingBottom: 40 }}>
						...
						/> */}
						<Text style={{ textAlign: "center", width: "100%", lineHeight: 26, ...fontSize(18), marginBottom: 24 }}>
							We need some additional information to provide you a better experience.
						</Text>
						<Text style={{ width: "100%", lineHeight: 26, ...fontSize(18), color: "#6B6B6B", marginBottom: 1 }}>{`Years you were at ${
							Account.tempData().name
							}`}</Text>
						<View style={{ flexDirection: "row", justifyContent: "space-between", paddingBottom: 15 }}>
							<TextField
								...
								onChange={(text: string) => {
									this.setState({ fromYear: text, error: { ...this.state.error, fromYear: { error: false, message: "" } } });
									this.navBar._hide();
								}}
							/>
							<TextField
								...
								returnKeyType="next"
								onChange={(text: string) => {
									this.setState({ toYear: text, error: { ...this.state.error, toYear: { error: false, message: "" } } });
									this.navBar._hide();
								}}
							/>
						</View>
						<TextField
							...
							returnKeyType="next"
							onChange={(text: string) => {
								this.setState({ major: text, error: { ...this.state.error, major: { error: false, message: "" } } });
								this.navBar._hide();
							}}
						/>
						<TextField
							...
							returnKeyType="done"
							onChange={(text: string) => {
								this.setState({ minor: text, error: { ...this.state.error, minor: { error: false, message: "" } } });
								this.navBar._hide();
							}}
						/>
						<TextField
						...
						/>
						<SubmitButton style={{ marginTop: 32 }} text="Send Request to Join" onPress={() => { }} />
					</View>
				</KeyboardAwareScrollView>
				</View>
			</SafeAreaView>
