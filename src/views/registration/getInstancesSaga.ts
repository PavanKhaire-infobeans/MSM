...
export const isCueBackInstance = (kAdmin == 'https://admin.cueback.com' ? true : false);
...
function* fetchInstances() {
    try {
        if(Utility.isInternetConnected){                
            ...
            let storedData = yield call((async function () { return await AsyncStorage.getItem(kStoreInstance) }));
            if (storedData) {
                console.log("Stored Data : ", storedData);
                yield put({ type: GetInstances.GetSuccess, payload: JSON.parse(storedData) })
            }
            var response: any = yield call((async function () { return await request }))
            ...
            if (response.ResponseCode == 200) {                                
                ...
                yield put({ type: GetInstances.GetFailed, payload: getValue(response, ["ResponseMessage"]) })
            }
        } else{
            No_Internet_Warning()
        }
