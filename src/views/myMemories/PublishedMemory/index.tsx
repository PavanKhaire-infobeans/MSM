...
import { SafeAreaView, ImageBackground, RefreshControl, ActivityIndicator, View, FlatList, Dimensions, StyleSheet, Image, TouchableHighlight, TouchableOpacity, DeviceEventEmitter, Alert, Keyboard, Platform, Animated } from "react-native";
import { Colors, fontSize, encode_utf8, MemoryActionKeys } from '../../../common/constants';
import { GetPublishedMemories, MemoryAction, kPublishedMemoriesFetched, kAllLikes, kLiked, kUnliked, kMemoryActionPerformed, kUpdateMemoryOnPublised , kUpdateMemoryOnTimeline, kMemoryActionPerformedPublished} from '../myMemoriesWebService';
...
import { Border, LikeCommentShare } from '../../memoryDetails/componentsMemoryDetails'
import {
    icon_like,
    icon_like_selected,
    icon_comment, profile_placeholder, icon_send, greenDotsButton, delete_memory, edit_memory, move_to_draft, remove_me_from_this_post, block_memory, block_user, cancelActions, report_user, block_and_report, add_icon, add_icon_small
} from '../../../images';
...
export default class PublishedMemory extends React.Component<Props, State> {
    ...
    constructor(props: Props) {
        ...
        this.publishedMemoryListener = EventManager.addListener(kPublishedMemoriesFetched, this.publishedMemoriesDetails);
        this.publishedMemoryUpdatedLister = EventManager.addListener(kPublishedMemoryUpdated, this.publishedMemoryUpdated)
        this.getAllLikesListener = EventManager.addListener(kAllLikes, this.allLikesFetched);
        this.memoryActionsListener = EventManager.addListener(kMemoryActionPerformedPublished, this.memoryActionCallBack)
    }
    ...
    componentDidMount() {
        ...
        this.memoryPublishedUpdateListener = EventManager.addListener("memoryUpdatePublishedListener", ()=>{
                this.setState({
                    isMemoryUpdate: true,
                });
                GetPublishedMemories("");
    ...
    publishedMemoryUpdated = (nid: any, likeCount: any, commentCount: any, likeFlag: any) => {
    ...
    handleLoadMore = () => {
        ...
        if (publishedMemoriesArray.length > 0 && publishedMemoriesArray.length < draftCount) {
    ...
    publishedMemoriesDetails = (fetched: boolean, publishedMemories: any) => {
        ...
        if (fetched) {
            ...
            if (publishedMemoriesArray.length == 0) {
                this.publishedMemoryDataModel.setPublishedMemoriesDetails(publishedMemories, true);
            } else {
                this.publishedMemoryDataModel.setPublishedMemoriesDetails(publishedMemories, false);
            }
            // if (page == 0){
            ...
            publishedMemoriesArray = this.publishedMemoryDataModel.getPublishedMemories()
        } else {
            // if(page != 0){
            //     page--;
            // }
                ...
                <SafeAreaView style={{ flex: 1, alignItems: "center", justifyContent: 'center', height: '100%' }}>
                    <View style={{ height: "100%", width: "100%" }}>
                        ...
                        {   publishedMemoriesArray.length == 0 &&
                            <View style={{position: 'absolute' ,height : '100%', width: '100%', alignContent: 'center', justifyContent: 'center', backgroundColor: 'white', padding : 16}}>
                                    {loadingDataFromServer ? <ActivityIndicator color={Colors.ThemeColor}    
                                                                size="large"                    
                                                                style={{flex: 1, justifyContent : "center"}}/> : 
                                                              <Text style={{...fontSize(18), color : "#909090", textAlign: 'center'}}>There are no memories available to display at this moment</Text>}
                            </View>    
                        }
                    </View>
...
export const MemoryActionsList = (item : any) =>{
    ...
    for (var value in item.actions_on_memory) {
        ...
        switch(value){
            case MemoryActionKeys.addToCollection: 
                memoryActions.push( {index: i, text: item.actions_on_memory[value], image: add_icon_small , nid: item.nid, memoryType: item.type,  actionType: MemoryActionKeys.addToCollection, uid : item.user_details.uid})     
                break;
            case MemoryActionKeys.blockUserKey: 
                memoryActions.push( {index: i, text: item.actions_on_memory[value], image: block_user, nid: item.nid, memoryType: item.type,  actionType: MemoryActionKeys.blockUserKey, uid : item.user_details.uid, isDestructive: 1})     
                break;
            case MemoryActionKeys.reportMemoryKey: 
                memoryActions.push( {index: i, text: item.actions_on_memory[value], image: report_user, nid: item.nid, memoryType: item.type,  actionType: MemoryActionKeys.reportMemoryKey, isDestructive: 1})     
                break;
            case MemoryActionKeys.blockAndReportKey: 
                memoryActions.push( {index: i, text: item.actions_on_memory[value], image: block_and_report, nid: item.nid, memoryType: item.type,  actionType: MemoryActionKeys.blockAndReportKey, uid : item.user_details.uid, isDestructive: 1})     
                break;         
            case MemoryActionKeys.editMemoryKey: 
                memoryActions.push( {index: i, text: item.actions_on_memory[value], image: edit_memory, nid: item.nid, memoryType: item.type,  actionType: MemoryActionKeys.editMemoryKey})
                break;
            case MemoryActionKeys.deleteMemoryKey: 
                memoryActions.push( {index: i, text: item.actions_on_memory[value], image: delete_memory, nid: item.nid, memoryType: item.type,  actionType: MemoryActionKeys.deleteMemoryKey })
                break;
            case MemoryActionKeys.moveToDraftKey: 
                memoryActions.push( {index: i, text: item.actions_on_memory[value], image: move_to_draft, nid: item.nid, memoryType: item.type,  actionType: MemoryActionKeys.moveToDraftKey })
                break;
            case MemoryActionKeys.removeMeFromThisPostKey: 
                memoryActions.push( {index: i, text: item.actions_on_memory[value], image: remove_me_from_this_post, nid: item.nid, memoryType: item.type,  actionType: MemoryActionKeys.removeMeFromThisPostKey })
                break;
            case MemoryActionKeys.blockMemoryKey: 
                memoryActions.push( {index: i, text: item.actions_on_memory[value], image: block_memory, nid: item.nid, memoryType: item.type,  actionType: MemoryActionKeys.blockMemoryKey })
                break;
...
export const CommentBox = (item: any) => {
    return <TouchableHighlight underlayColor={"#ffffff00"} style={{ minHeight: 60, maxHeight: 300, width: "100%", flexDirection: "row", paddingRight: 15, paddingLeft: 5, justifyContent: "center", alignItems: "center", paddingBottom: 5 }} onPress={() => Actions.jump("memoryDetails", { "nid": item.item.nid, "type": item.item.type, "comment": true })}>
        <View style={{ flex: 1, flexDirection: "row", justifyContent: "center", alignItems: "center", paddingTop: 5}}>
            <Text  style={{ ...fontSize(18), color : Colors.dullText, flex: 1, borderWidth: 1, maxHeight: 100, borderColor: "rgba(55, 56, 82, 0.2)", marginTop: 0, marginLeft: 10,margin: 10, borderRadius: 5, padding: 10, alignItems: "center", backgroundColor: "rgba(55, 56, 82, 0.1)" }}
            >Write a comment..</Text>

            <TouchableOpacity style={{ alignItems: "center", justifyContent: "center" }}>
                <Image source={icon_send} />
                <Text style={{ fontSize: 12, textAlign: "center", color: Colors.ThemeColor, padding: 1 }} autoCorrect={false}>{"Post"}</Text>
            </TouchableOpacity>
        </View>
    </TouchableHighlight>
}
export const RenderLikeAndCommentSection = (item: any, like : any, animate : any) => {
    ...
    if(memoryDetail.noOfComments > 0 || memoryDetail.showLikeCount){
        if(memoryDetail.showLikeCount && memoryDetail.noOfLikes > 0){
            likes =  memoryDetail.noOfLikes.toString().concat(memoryDetail.noOfLikes > 1 ? " Likes" : " Like")
        }
        if (memoryDetail.noOfComments > 0){
            comments = memoryDetail.noOfComments.toString().concat(memoryDetail.noOfComments > 1 ? " Comments" : " Comment")
        }            
    }
    return <View style={{ paddingRight: 15, paddingLeft: 15 }} key= {item.index}>
        {(memoryDetail.noOfComments > 0 || memoryDetail.showLikeCount) && <Border />}
        <View style={{ flexDirection: "row", justifyContent: "space-between", marginBottom: 5 }}>
           <TouchableHighlight underlayColor={"#ffffff00"} style={{ flexDirection: "row", flex: 1, justifyContent: "flex-start", alignItems: "center" }}
                onPress={() => {
                    EventManager.callBack("startAnim");
                    like(item);
                }}
            >
              <LikeCommentShare id={item.index} animate={animate} animateType={"like"} selectedItem={memoryDetail.isLikedByUser} name= {likes} icon={memoryDetail.isLikedByUser ? icon_like_selected : icon_like} />
            </TouchableHighlight>
            <TouchableHighlight underlayColor={"#ffffff00"} style={{ flexDirection: "row", flex: 1, justifyContent: "flex-end", alignItems: "center" }}
                onPress={() => {
                    Actions.jump("memoryDetails", { "nid": item.item.nid, "type": item.item.type, "comment": true })
                }}
            >
                <LikeCommentShare id={item.index} animate={animate} animateType={"comment"} selectedItem={memoryDetail.isLikedByUser} name={comments} icon={icon_comment} />
            </TouchableHighlight>
        </View>
    </View>
...
export const _onOpenPdfs=(pdfUrl: any)=>{
    if (Utility.isInternetConnected) {
        Actions.jump("pdfViewer", { file: { url: encode_utf8(pdfUrl) } });
    } else {
        No_Internet_Warning();
    }
}
...
export const MemoryBasicDetails = (userDetails: any, item: any, openMemoryActions : any, listType : any) => {
    return (
            <View style={{ width: "100%", alignItems: "center", justifyContent: "space-between", flexDirection: "row" }}>
                <View style={{flexDirection: "row", paddingLeft: 14}}> 
                    <View style={{ padding: 5,  flexDirection: "row", alignItems: "center" }}>
                        <View>
                            <Text style={{ ...fontSize(16), fontFamily: "Rubik", color: Colors.TextColor, lineHeight: 20}}>{"By "}<Text style={{ color: Colors.NewYellowColor, fontWeight: Platform.OS === "ios"? '500':'bold', lineHeight: 20 }}>{userDetails.name}</Text></Text>
                            {listType == ListType.Published && <Text style={{ ...fontSize(14), fontFamily: "Rubik", color: Colors.TextColor, lineHeight: 20}}>{Utility.timeDuration(""+item.created, "M d, Y")}{item.viewCount > 0 && <Text>{" | "}{item.viewCount}{(item.viewCount > 1 ? " views" : " view")}</Text>}</Text>}
                        </View>
                    </View>
                </View>
                    <TouchableOpacity
                        style={{padding: 16, height: "100%", maxHeight: 70, alignItems: "center", justifyContent: "center"}}
                        onPress={() => {
        ...
        <View>
            {
                memoryDetail.images && memoryDetail.images.length > 0 ?
                    ...
                    <View style={{ margin: 16, marginTop: 0, height: 117, flexDirection: "row", justifyContent: "space-around" }}>
                        <TouchableHighlight underlayColor={"#ffffff00"}
                            style={{ flex: 1 }}
                            onPress={
                                () => {
                        ...
                        >
                            <PlaceholderImageView
                                style={{ backgroundColor: Colors.NewLightThemeColor, flex: 1, marginRight: 2 }}
                                uri={Utility.getFileURLFromPublicURL(memoryDetail.images[0].thumbnail_url)}
                                resizeMode={"contain"} />
                        </TouchableHighlight>
                        ...
                        {memoryDetail.images.length > 1 &&
                            <View style={{ flex: 1, backgroundColor: Colors.NewLightThemeColor}}>
                                ...
                                <TouchableHighlight underlayColor={"#ffffff00"}
                                    ...
                                    <View style={{flex: 1}}><PlaceholderImageView
                                        style={{ flex: 1, marginLeft: 2 }}
                                        uri={Utility.getFileURLFromPublicURL(memoryDetail.images[1].thumbnail_url)}
                                        resizeMode={"contain"} />

                                        {memoryDetail.images.length > 2 && <View style={{ backgroundColor: "rgba(0, 0, 0, 0.6)", height: "100%", width: "100%", position: "absolute", top: 0, alignItems: "center", justifyContent: "center" }}>
                                            <Text style={{ color: "#fff", ...fontSize(16), textAlign: "center" }}>{"+"}{memoryDetail.images.length - 2}{"\n more"}</Text>
                                        </View>}
                                    </View>
                                </TouchableHighlight>
            ...
            {audioView(memoryDetail)}
            {
                memoryDetail.pdf && memoryDetail.pdf.length > 0 ?
                    <View style={{ margin: 16, marginTop: 0, height: 117, flexDirection: "row", justifyContent: "space-around" }}>
                        <TouchableHighlight underlayColor={"#ffffff00"}
                            style={{ backgroundColor: Colors.NewLightThemeColor, flex: 1, marginRight: 2, height: "100%" }}
                            onPress={
                                () => {
                        ...
                        >
                            <PlaceholderImageView
                                ...
                                openPDF={true}
                                uri={Utility.getFileURLFromPublicURL(memoryDetail.pdf[0].pdf_image_url)}
                                resizeMode={"contain"} />
                        </TouchableHighlight>
                        {memoryDetail.pdf.length > 1 &&
                            <View style={{ flex: 1, backgroundColor: Colors.NewLightThemeColor }}>
                                ...
                                >
                                    <View><PlaceholderImageView
                                        style={{ flex: 1, marginLeft: 2, height: "100%" }}
                                        openPDF={true}
                                        uri={Utility.getFileURLFromPublicURL(memoryDetail.pdf[1].pdf_image_url)}
                                        resizeMode={"contain"} />
                                        {memoryDetail.pdf.length > 2 && <View style={{ backgroundColor: "rgba(0, 0, 0, 0.6)", height: "100%", width: "100%", position: "absolute", top: 0, alignItems: "center", justifyContent: "center" }}>
                                            <Text style={{ color: "#fff", ...fontSize(16), textAlign: "center" }}>{"+"}{memoryDetail.pdf.length - 2}{"\n more"}</Text>
                                        </View>}
                                    </View>
                                </TouchableHighlight>
                            </View>
