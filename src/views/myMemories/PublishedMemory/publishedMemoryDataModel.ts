...
import { getValue, getDetails, keyInt, keyArray, keyString, keyObject } from "../../../common/constants";
...
export class PublishedMemoryDataModel {
    ...
    getDetails(publishedMemories, ["data"], keyArray).forEach((element : any) => {
        let publishedMemory: any = {};
        publishedMemory.actions_on_memory = getDetails(element, ['actions_on_memory'] , keyObject); 
        publishedMemory.comments_count = getDetails(element, ['comments_count'] , keyString); 
        publishedMemory.created = parseInt(getDetails(element, ['created'] , keyInt)); 
        publishedMemory.description = getDetails(element, ['description'] , keyString); 
        publishedMemory.noOfComments = getDetails(element, ["comments_count"], keyInt);
        publishedMemory.noOfLikes = getDetails(element, ["like_comment_data", "like_count"], keyInt);
        publishedMemory.viewCount = getDetails(element, ["view_count"], keyInt); 
        publishedMemory.isLikedByUser = getDetails(element, ["like_comment_data", "like_flag"], keyInt);
        publishedMemory.location = getDetails(element, ['location'] , keyString); 
        publishedMemory.memory_date = getDetails(element, ['memory_date'] , keyString); 
        publishedMemory.mins_to_read = getDetails(element, ['mins_to_read'] , keyString); 
        publishedMemory.nid = getDetails(element, ['nid'] , keyString); 
        publishedMemory.season = getDetails(element, ['season'] , keyString); 
        publishedMemory.share_count = getDetails(element, ['share_count'] , keyInt); 
        ...
        publishedMemory.user_details = getDetails(element, ['user_details'] , keyObject); 
        ...
        publishedMemory.memory_date = Utility.dateObjectToDefaultFormat(new Date(parseInt(getDetails(publishedMemory, ["memory_date"])) * 1000));                    
        if(publishedMemory.season && publishedMemory.season.trim().length > 0){
            let season = publishedMemory.season.trim();
            publishedMemory.memory_date = Utility.dateAccordingToFormat(""+publishedMemory.memory_date, "Y");  
            publishedMemory.memory_date = season.charAt(0).toUpperCase() + season.slice(1) + " " + publishedMemory.memory_date;
        } else{
            publishedMemory.memory_date = Utility.dateAccordingToFormat(""+publishedMemory.memory_date, "M Y");
        }
        if (publishedMemory.location){
            publishedMemory.dateWithLocation = publishedMemory.memory_date+", " + publishedMemory.location
        }
        
        const regex = /(<([^>]+)>)/ig;
        publishedMemory.description = publishedMemory.description.replace(regex, '');
