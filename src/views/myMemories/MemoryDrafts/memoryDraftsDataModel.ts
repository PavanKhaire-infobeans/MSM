...
import { getValue, getDetails, keyInt, keyArray, TimeStampMilliSeconds } from "../../../common/constants";
...
export class MemoryDraftsDataModel {   
    ...
    updateMemoryDraftObj(memoryDraftDetails: any , shouldReplace: boolean){
        ...
        getDetails(memoryDraftDetails, ["data"], keyArray).forEach((element : any) => {
            let memoryDraft: any = {};
            memoryDraft.activity = getDetails(element, ["activity"]);        
            memoryDraft.attachment_count = getDetails(element, ["attachment_count"], keyInt);        
            memoryDraft.changed = Utility.dateObjectToDefaultFormat(new Date(parseInt(getDetails(element, ["changed"])) * 1000));                                            
            memoryDraft.collaborators = getDetails(element, ['collaborators'] , keyArray); 
         
            memoryDraft.created = parseInt(getDetails(element, ["created"]));                    
            memoryDraft.created = Utility.timeDuration(""+memoryDraft.created, "M d, Y");

            memoryDraft.field_is_draft_value = getDetails(element, ["field_is_draft_value"]);        
            memoryDraft.image_path = getDetails(element, ["image_path"]);        
            memoryDraft.my_chat_count = getDetails(element, ["my_chat_count"], keyInt);        
            memoryDraft.new_attachment_count = getDetails(element, ["new_attachment_count"]);        
            memoryDraft.new_collaborator_count = getDetails(element, ["new_collaborator_count"], keyInt);        
            memoryDraft.nid = getDetails(element, ["nid"]);        
            memoryDraft.title = getDetails(element, ["title"]);        
            memoryDraft.unread_chat_count = getDetails(element, ["unread_chat_count"]);
            memoryDraft.journal_name = getDetails(element, ["journal_name"]);
        
            if(Account.selectedData().userID!=element.uid){
                let first_name = getDetails(element.user_details, [ "field_first_name_value"]);
                let last_name = getDetails(element.user_details, [ "field_last_name_value"]);
            ...
            if (uri != undefined  && uri != null && uri.trim().length != 0){
                memoryDraft.userProfilePic = element.user_details["uri"] ? Utility.getFileURLFromPublicURL(uri) : ""
            }
    ...
    getEditContentObject(draftDetails: any){
        ...
        let memoryDate = new Date(parseInt(getDetails(draftDetails, ["memory_date"])) * 1000);                    
        ...
        let tags = getDetails(draftDetails, ["memory_tags"]).map((element : any, index : any)=>{
                    return {tid : TimeStampMilliSeconds()+index,name : element}})
        let shareOption = getDetails(draftDetails, ["share_option_value"]).length > 0 ? getDetails(draftDetails, ["share_option_value"]) : "allfriends" ;
        let images : any = [];
        ...
        let whoElseWhereThere = getDetails(draftDetails, ["who_else_was_there"], keyArray)
        let youWhereThere =  whoElseWhereThere.some((element : any) => element.uid === Account.selectedData().userID);      
        let taggedCount = youWhereThere ? whoElseWhereThere.length - 1 : whoElseWhereThere.length;
        let etherpad_details = getDetails(draftDetails, ["etherpad_details"])        
        let collaboratorsGroups = getDetails(draftDetails, ["collaborators", "groups"], keyArray)
        let collaboratorsUsers = getDetails(draftDetails, ["collaborators", "users"], keyArray)
        let details : any = { isCreatedByUser :  getDetails(draftDetails, ["user_details"]),
                             etherpad_details : {padId : getValue(etherpad_details, ["padid"]),
                             ...
                             whoCanSeeMemoryUids: shareOption == "custom" ? getDetails(draftDetails, ["share_users"], keyArray) : [], 
                             whoCanSeeMemoryGroupIds : shareOption == "custom" ? getDetails(draftDetails, ["share_groups"], keyArray) : [], 
                             shareOption : shareOption,
                             files : images.concat(audios.concat(pdf)), 
                             collectionList : [], 
                             collections: getDetails(draftDetails, ["collections"]),
                             collection: {name : getDetails(draftDetails, ["collection_name"]), 
