...
function* getListFlow(requestData: any) {
	...
	try {
		...
		let request = yield call(getListCall, [rData, { "X-CSRF-TOKEN": data.userAuthToken, "Content-Type": "application/json" }]);
		...
		responseBody.Details.mindPopList.forEach((element: any, index: any) => {
			responseBody.Details.mindPopList[index].message = encode_utf8(responseBody.Details.mindPopList[index].message)
		}); 
		...
		if (fetchedItems.rows.length > 0) {
			...
			yield put({ type: GetMindPopStatus.RequestSuccess, payload: { count: value, fetchedItems } });
		} else {
			yield put({ type: GetMindPopStatus.RequestSuccess, payload: { count: 0, fetchedItems: [] } });
		...
		if (fetchedItems.rows.length > 0) {
			yield put({ type: GetMindPopStatus.RequestSuccess, payload: { count: fetchedItems.rows.length, fetchedItems } });
		} else {
