...
const styles = EStyleSheet.create({
    ...
    rightButtonsBadge: {
        position: 'absolute', height: 16, right: 5, top: 5,
        backgroundColor: "#ff0000", borderColor: '#ffffff',
        borderWidth: 1, borderRadius: 8, alignContent: 'center'
    },
...
class MindPopNavigationBar extends React.Component<{ [x: string]: any }> {
    ...
    _renderLeft() {
        ...
        return (
            <TouchableOpacity style={[styles.leftButtonTouchableContainer]} testID={testID.leftButtons.menu}
                onPress={() => action()}>
                <Image
    ...
    _renderRight() {
        var action = this.props.isSelectionOn
            ? (this.props.selectedItemCount == this.props.listCount ? this.props.clearAllAction : this.props.selectAllAction)
            : this.props.selectAction
        var title = this.props.isSelectionOn
            ? (this.props.selectedItemCount == this.props.listCount ? "Clear All" : "Select All")
            : "Select"
    ...
    render() {
        return (
            <View style={{flexDirection: "row", backgroundColor:Colors.NewThemeColor, height: 54}} >
