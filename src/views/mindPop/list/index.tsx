...
import {
  GetMindPopStatus,
  MindPopListCount,
  MindPopListSelectionState,
  MindPopSelectedItemCount,
} from './reducer';
//@ts-ignore
...
import {
  ...
  default as loaderHandler,
} from '../../../common/component/busyindicator/LoaderHandler';
...
class MindPopList extends React.Component<{
  listMindPops: (payload: any) => void;
  [x: string]: any;
}> {
  state: {
  ...
  constructor(props: {
    [x: string]: any;
    listMindPops: (payload: any) => void;
    fromDeeplinking?: boolean;
    nid?: any;
    deepLinkBackClick?: boolean;
  }) {
    super(props);
    this.eventSubs = DeviceEventEmitter.addListener(
      'updateSelected',
      this._updateIndex,
    );
  ...
  _populateListFromDB(
    results: any[],
    reloadList: (sectionData: SectionItems) => void,
  ) {
    ...
    updatedArray.sort((obj1: ListItem, obj2: ListItem): number => {
  ...
  UNSAFE_componentWillReceiveProps(nextProps: {[x: string]: any}) {
    if (nextProps.list.completed) {
      ...
      if (nextProps.deleteStatus.success) {
        ...
        if (this.props.navigation.currentScene == 'mindPopList') {
  ...
  updateList = (searchKeyword: string = '') => {
    ...
    MindPopStore._getMindPopFromLocalDB(this.searchKeyword).then(
      (list: any[]) => {
        this._populateListFromDB(list, updatedSectionListData => {
          let list =
            updatedSectionListData[updatedSectionListData.length - 1].data;
          if (list.length > 0) {
            this.setState(
              {
                listSectionItems: updatedSectionListData,
    ...
    <View
      ...
      <Text
        style={{
          ...
          fontWeight: '600',
          fontFamily:
            Platform.OS === 'ios' ? fontFamily.Inter : fontFamily.IntersemiBold,
        }}>
        No Results Found
      </Text>
      <Text
        style={{
          ...
          marginTop: 16,
          textAlign: 'center',
        }}>{`We couldn't find any results for '${this.searchKeyword}'. Try searching for something else`}</Text>
    </View>
  );
              ...
              onPress={() => {
                if (DeviceInfo.isTablet()) {
                  ...
                  this.props.isSelectingItem
                    ? this._selectRow(data.item)
                    : this._editAction(item, data, true);
                }
              }}>
              <View style={styles.rowFront}>
                ...
                {!this.props.isSelectingItem && (
                  <TouchableOpacity
                    ...
                    <Text style={Styles.convertoMemory}>
                      {'Convert to Memory'}
                    </Text>
                  </TouchableOpacity>
                )}
      ...
      <TouchableOpacity
        ...
        <View
          ...
          <Text
            ...
            {(DeviceInfo.isTablet() ? '+' : '') +
              `Capture ${DeviceInfo.isTablet() ? 'a' : 'your'} MindPop` +
              (DeviceInfo.isTablet() ? '' : '...')}
          </Text>
        </View>
      </TouchableOpacity>
