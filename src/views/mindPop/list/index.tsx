...
class MindPopList extends React.Component<{ listMindPops: (payload: any) => void;[x: string]: any }> {
	...
	createMemoryCallBack = (success: boolean, draftDetails: any) => {
		...
		if (success) {
			this.props.navigation.replace("createMemory", { editMode: true, draftNid: draftDetails, deepLinkBackClick: this.props.deepLinkBackClick })
		}
		else {
			loaderHandler.hideLoader()
	...
	UNSAFE_componentWillReceiveProps(nextProps: { [x: string]: any }) {
		if (nextProps.list.completed) {
			...
			if (nextProps.deleteStatus.success) {
				...
				if (this.props?.navigation?.state?.routeName == "mindPopList") {
  ...
  _cancelAction = () => {
	...
	createNewMindPop = (fromNavBar?: boolean) => {
		...
		if (fromNavBar) {
			loaderHandler.showLoader("Loading...");
			this.props.navigation.navigate('mindPopEdit',{ updateList: this.updateList, actionImageUpload: this.props.actionImageUpload, actionRecord: this.props.actionRecord, actionWrite: this.props.actionWrite });
		} else {
		      this.props.navigation.navigate('mindPopEdit', {
		        updateList: this.updateList,
		      });
