...
import { View, Image, SafeAreaView, TouchableOpacity, Alert, ActivityIndicator, Keyboard } from "react-native";
...
type Props = { selectedItem: { uri?: string; filePath?: string; isLocal?: boolean; thumb_uri?:string; }; deleteItem: Function; reset: Function; isEditMode: boolean };
...
export default class ImagePreview extends React.Component<Props, State> {	
	...
	private _errorView: JSX.Element = (
		<View style={{ flex: 1, alignItems: "center", justifyContent: "center" }}>
			<Text style={{ ...fontSize(14), color: "black" }}>Image could not be loaded</Text>
		</View>
	);
	...
	render() {
		...
		let uri = !getValue(this.props.selectedItem, ["isLocal"]) ? this.props.selectedItem.uri : this.props.selectedItem.filePath
		return (
			<SafeAreaView style={{ flex: 1, justifyContent: "center", alignItems: "center", backgroundColor: Colors.NewLightThemeColor }}>
				...
				<View
					...
					{this.props.selectedItem.isLocal ? null : (
						<TouchableOpacity
							...
							{this.props.isEditMode ? <Image source={rubbish} resizeMode="contain" /> : null}
						</TouchableOpacity>
					)}
					...
					<TouchableOpacity
						...
						<Text style={{ ...fontSize(18), color: Colors.NewTitleColor }}>Close</Text>
					</TouchableOpacity>
				</View>
			</SafeAreaView>
		);
