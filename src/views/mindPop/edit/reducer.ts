...
export const addMindPop = (state: ResultType = {}, action: Payload): ResultType => {
	switch (action.type) {
...
export const editScreenMode = (state: State = { mode: false, selectedMindPop: null }, action: Payload): State => {
	switch (action.type) {
		case EditMode.EDIT:
			return { mode: true, selectedMindPop: action.payload || (state.selectedMindPop || null) };
		case EditMode.RESET:
			return { mode: false, selectedMindPop: action.payload || (state.selectedMindPop || null) };
        case EditMode.SET:
...
function* fetchAttachments(...args: any[]) {
	...
	if (action.payload) {
        ...
        for (let media of medias) {
            ...
            extraData =  {...extraData, [key]: [...(extraData[key] || []), Convert(media)] }
        }
...
export function* checkAttachments() {
	yield all([takeLatest(EditMode.EDIT, fetchAttachments), takeLatest(EditMode.RESET, fetchAttachments)]);
}
