import { getValue, Storage, TimeStampMilliSeconds, uploadTask, ERROR_MESSAGE, asyncGen, MindPopsInProgress, decode_utf8 } from '../../../common/constants';
import MindPopStore from "../../../common/database/mindPopStore/mindPopStore";
import EventManager from '../../../common/eventManager';
import { Account } from '../../../common/loginStore';
import { addMindPops, getMindPopWithId } from '../../../common/webservice/mindPopServices';
 ...
 */
export const addEditMindPop = async (param: any, files: TempFile[] = [], uploadInBackgroud? : boolean) => {
    try {
        let data = await Storage.get('userData')
        let response = await (() => {
            return addMindPops(`https://${Account.selectedData().instanceURL}`, param, { "X-CSRF-TOKEN": data.userAuthToken, "Content-Type": "application/json" }).then((response: Response) => response.json()).catch((err: Error) => Promise.reject(err));
        })()
        let mindpopId = parseInt(getValue(response, ["MindpopId"])) || 0
        if (mindpopId == 0) {
            EventManager.callBack(kAddEditIdentifier, false, "Could not save MindPop");
            return;
        } 
        ...
        let responseList = await (() => {
            return getMindPopWithId(`https://${Account.selectedData().instanceURL}`, [listRequestParams, { "X-CSRF-TOKEN": data.userAuthToken, "Content-Type": "application/json" }])
        ...
        if (value != null && typeof value !== "undefined") {
            ...
            if (item.length > 0) {
                ...
                EventManager.callBack(kAddEditIdentifier, false, "Unable to load MindPop");
            }
        } else {
            ...
            loaderHandler.hideLoader();
            EventManager.callBack(kAddEditIdentifier, false, getValue(err, ["message"]) || ERROR_MESSAGE);
