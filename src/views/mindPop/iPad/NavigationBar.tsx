...
const testID = {
	...
	rightButtons: { mindpop: "mindpop_btn", message: "message_btn", notification: "notification_btn" },
	title: { text: "title" }
};
...
const styles = EStyleSheet.create({
	...
	rightButtonsContainer: { flex: 1, paddingTop: 10, paddingRight: 5, flexDirection: "row", justifyContent: "flex-end", alignItems: "center" },
...
class MindPopIPadNavigationBar extends React.Component<{ [x: string]: any }> {
	_renderRight() {
		...
		let title = this.props.isSelectionOn ? (this.props.selectedItemCount == this.props.listCount ? "Clear All" : "Select All") : "Select";

		return (
			<View style={styles.rightButtonsContainer}>
				<TouchableOpacity onPress={() => action()} style={styles.rightButtonsTouchable} testID={testID.rightButtons.mindpop}>
					<Text style={{ ...fontSize(16), fontWeight: "400", color: Colors.TextColor }}>{title}</Text>
				</TouchableOpacity>
			</View>
		);
	}
	...
	render() {
		let action = this.props.isSelectionOn ? this.props.backAction : this.props.cancelAction;
		let leftImg = this.props.isSelectionOn ? backBtn : navBarCrossIconWhite;
		return (
			<NavigationHeader testID={testID.dashboardNavBar} backgroundColor={Colors.NewThemeColor}>
				<StatusBar barStyle="dark-content" backgroundColor = {Colors.NewThemeColor}/>
				<View style={{ width: this.props.listCount > 0 ? 320 : "100%", height: "100%", backgroundColor: "transparent", flexDirection: "row" }}>
					<TouchableOpacity style={styles.leftButtonTouchableContainer} testID={testID.leftButtons.menu} onPress={() => action()}>
						<Image style={{ height: 28, width: 28 }} resizeMode="center" source={leftImg} />
					</TouchableOpacity>
				...
				{this.props.listCount > 0 ? (
					<React.Fragment>
						<View
							key="sepeartor"
							style={{ width: 1, backgroundColor: "#fff", height: Platform.OS === "ios" ? 76 : 54, top: Platform.OS === "ios" ? -20 : 0 }}
						/>
						<View
							key="rightSection"
							style={{
							...
							<TouchableOpacity
								...
								<Text style={{ ...fontSize(16), fontWeight: "400", color: Colors.TextColor }}>+Create a MindPop</Text>
							</TouchableOpacity>
							<TouchableOpacity
								onPress={() => {
									this.props.saveMode();
								...
								testID={"headerCreateMindPop"}>
								<Text style={{ ...fontSize(16), fontWeight: "400", color: Colors.TextColor }}>Edit</Text>
							</TouchableOpacity>
                            {this.props.isSelectingItem ? (
                                <View style={{ height: (Platform.OS === "ios" ? 76 : 54), top: Platform.OS == "ios" ? -20 : 0, width: "100%", position: "absolute", backgroundColor: "#00000034" }} />
                            ) : null}
						</View>
					</React.Fragment>
				) : null}
