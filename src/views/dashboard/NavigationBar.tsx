import { Image, Platform, StyleSheet, TouchableOpacity, View, StatusBar, Alert, ImageBackground , DeviceEventEmitter, Keyboard} from 'react-native';
import Text from '../../common/component/Text';
import React from 'react';
import { Actions } from 'react-native-router-flux';
import { mindpopBarIcon, close_white, instanceLogo, profile_placeholder, cueback_logo, white_head_icon, icon_notification} from '../../images'
//@ts-ignore
...
import TabIcon, { isNotification, kNotificationIndicator } from '../../common/component/TabBarIcons';
...
const testID = {
  ...
  rightButtons: { mindpop: 'mindpop_btn', message: 'message_btn', notification: 'notification_btn' , notificationIcon : 'icon_notification'},
  title: { text: 'title' }
};
...
const styles = EStyleSheet.create({
  ...
  leftButtonContainer: {
    backgroundColor: 'transparent', borderColor: '#ffffff', borderWidth: 2,
    height: 28, width: 28, borderRadius: 14, alignItems: 'center',
    justifyContent: 'center',
  },
  leftButtonMSMContainer : {
    backgroundColor: 'transparent', 
    height: 28, width: 28, alignItems: 'center',
    justifyContent: 'center',
  },
  closeButton: {    
    height: 28, width: 28, alignItems: 'center',
  ...
  rightButtonsContainer: { flex: 1, paddingTop: 10, flexDirection: 'row', justifyContent: 'flex-end', alignItems: 'center' },
  ...
  rightButtonsBadge: {
    position: 'absolute', height: 15, width: 15, right: 10, top: 4,
    zIndex  : 9999,
    backgroundColor: "#ff0000", borderColor: '#ffffff',
    borderWidth: 2, borderRadius: 8, alignContent: 'center'
  },
...
class NavigationBar extends React.Component<Props> {
    ...
    return (
      <View style={styles.rightButtonsContainer}>
        ...
        <TouchableOpacity onPress={() => { 
          ...
          <Image style={styles.rightButtonsBackgroundImage} source={mindpopBarIcon} resizeMode='center' />
        </TouchableOpacity>
        <TouchableOpacity onPress={() => { 
        ...
        {this.props.unreadNot && <View style={styles.rightButtonsBadge}/>}
          <Image style={styles.rightButtonsBackgroundImage} source={icon_notification} resizeMode='center' />
        </TouchableOpacity>

        <TouchableOpacity onPress={() => this._userProfileSection()} style={styles.rightButtonsTouchable} testID={testID.rightButtons.notification}>
          <ImageBackground style={[styles.avatar]} imageStyle={{ borderRadius: 15}} source={profile_placeholder}>
              {
                loadProfileImage ? (
                  <Image style={styles.avatar} source={Account.selectedData().profileImage != "" ? {uri : Account.selectedData().profileImage} : profile_placeholder}/>
  ...
  render() {
    let showClose : boolean  = this.props.showClose ? this.props.showClose : false;  
    let isPublicInstance: any = Account.selectedData().is_public_site;
    return (
      <View style={{ flexDirection: "row", width: "100%", height: 54, justifyContent: "space-between", backgroundColor: this.props.isWhite ? "#fff" : Colors.NewThemeColor}}>
                <TouchableOpacity style={styles.leftButtonTouchableContainer} testID={testID.leftButtons.menu} onPress={() => { showClose ? this._closeAction() : Actions.drawerOpen() }}>
          {showClose ?<View style={styles.closeButton}>
            <Image
              source={close_white}
            />
          </View>:
