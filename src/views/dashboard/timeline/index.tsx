...
const Timeline = (props: Props) => {
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      ...
      let filteredList = [...new Set(allYearsTemp.map(JSON.stringify))].map(
        JSON.parse,
      );
  ...
  useEffect(() => {
    if (
      props.timelineList &&
      props.timelineList.length &&
      timelineBarNextPrevClick
    ) {
      let currentYear = props.timelineList[0].memoryYear;
      let next = '',
        prev = '',
        currentIndex = memoryYears.indexOf(currentYear);
      ...
      debugger;
      if (
        (indexToScroll >= 0 || indexToScroll < allYears.length) &&
        timelineYearRef?.current?.scrollToIndex
      ) {
        timelineYearRef?.current?.scrollToIndex({
          animated: false,
          index: indexToScroll,
  ...
  const handleLoadMore = () => {
    if (!props.isLoadMore) {
      ...
      props.fetchMemoryList({
        ...
        isLoadMore: true,
        jumpTo: true,
        lastMemoryDate: memoryDetails.memoryDate,
        filters: props.filters,
      });
    }
  };
      ...
      return (
        <View style={styles.audioViewContainerStyle}>
          ...
          {item.audios.length > 1 ? (
            <TouchableOpacity
              onPress={() => {
                _onShowMemoryDetails(item, navigation);
              }}
  ...
  const jumpToClicked = (selectedYear: any, selectedMonth: any) => {
    ...
    let next = '',
      prev = '',
      currentIndex = memoryYears.indexOf(selectedYear);
      ...
      <SafeAreaView style={styles.container}>
        <View style={styles.subcontainer}>
          ...
          <ActionSheet
            ...
            ref={actionSheetRef}>
            <JumpToScreen
              jumpToClick={(selectedYear: any, selectedMonth: any) =>
                jumpToClicked(selectedYear, selectedMonth)
              }
              jumpToYears={props.jumpToYears}
              memoryDate={state.jumpToDate}
                ...
                <>
                  {currentItemYear == allYears[allYears.length - 1].year ? (
                    ...
                    <TouchableHighlight
                      onPress={() => {
                        let allYearsArray = [...allYears];
                        let indexToScroll = allYearsArray.findIndex(
                          x => x.year == currentItemYear,
                        );
          ...
          <View
            style={{
              height: allYears.length && scrolling ? 2 : 0,
              width: '100%',
          ...
          <FlatList
            ...
            onScroll={e => {
              ...
              if (
                ...
                e?.nativeEvent?.contentOffset?.y > 9
              ) {
                setScrolling(true);
              }
              if (
