...
import { View, TouchableOpacity, SafeAreaView, TextInput, TouchableWithoutFeedback, DeviceEventEmitter, Keyboard, Animated, StatusBar, Dimensions, Modal, Image, TouchableHighlight, Alert, Platform } from "react-native";
...
import { LoginControllerProtocol, LoginController, LoginViewProtocol, Props } from "./loginController";
import { connect } from "react-redux";
import { LoginState, LoginServiceStatus, LoginInstanceStatus } from "./loginReducer";
...
import CommonInstanceListsSelection, { ListType } from "./commonInstanceListSelection";
// @ts-ignore
...
class Login extends React.Component<Props> implements LoginViewProtocol {
	...
	constructor(props: Props) {
		...
		DeviceEventEmitter.addListener("AppleLoginResult", this.showAlertFromNative)
		this.controller.onClickAppleSignIn = this.controller.onClickAppleSignIn.bind(this.controller)
		this.controller.onClickGoogleSignIn = this.controller.onClickGoogleSignIn.bind(this.controller)
		this.appleLoginCallBack =  EventManager.addListener(kRegSignUp, this.regSignUpListener.bind(this));            
	}

	regSignUpListener=(type : any)=>{
		switch(type){
			case loginType.appleLogin : this.controller.onClickAppleSignIn()
				break;
			case loginType.googleLogin : this.controller.onClickGoogleSignIn()
				break;	
		}		
	}
		...
		return (
			<SafeAreaView style={styles.container}>	
				...
				<Modal            
          			...
          			onRequestClose = {() => this.updateState({...this.state, isVisible: false}) }>            
              		<CommonInstanceListsSelection username={this.state.username} onClick={(selectedCommunity : any)=> this.loginToSelected(selectedCommunity)} type={ListType.Login} title={"Login"} listAccounts={this.state.instanceData} onRequestClose={()=> this.updateState({...this.state, isVisible: false})} isDisabledAccount={this.state.isDisabledAccount}/>  
					...
					<View style={styles.innerContainer}>
						<View style={styles.loginContainer}>
							<KeyboardAwareScrollView 
								...
								style={{width: Dimensions.get('window').width, height: '100%', padding: 15}}>
								{/** Commuity banner UI */}
								...
								<TextField
									...
									returnKeyType="next"
									onChange={(text: any) => this.controller.onTextChange("username", text)}
								/>
								<TextField
									passwordToggle={true}
									...
									returnKeyType="go"
									onChange={(text: any) => this.controller.onTextChange("password", text)}
								/>
								{/** Sign In section */}
								<TouchableHighlight underlayColor={"#ffffffff"}
										style={styles.forgotPassword}
										onPress={() => this.setState({_isRemeberMe: !this.state._isRemeberMe})}>
										<View style={[styles.forgotPasswordContainer, {flexDirection: "row", alignItems: "center", justifyContent: "space-between"}]}>
											{/* <Image source={this.state._isRemeberMe ? checkbox_active : checkbox} style={{height: 14, width: 14}}></Image> */}
												<Text style={{ fontWeight: Platform.OS === "ios"? '600':'bold', color: Colors.TextColor, ...fontSize(Size.byWidth(16))}}>Remember Me</Text>
												<ToggleSwitch
													isOn={this.state._isRemeberMe}
													onColor={Colors.NewTitleColor}
													offColor="#E5E5E5"													
													labelStyle={{ color: "black", fontWeight: Platform.OS === "ios"? '900':'bold' }}
													size="medium"
													onToggle={(isOn: any)=>{this.setState({_isRemeberMe : isOn})}}
													/>	
										</View>										
								</TouchableHighlight>
								<SubmitButton style={{backgroundColor: Colors.NewTitleColor, ...fontSize(22)}} text="Login" onPress={this.controller.onClick.bind(this.controller)} />
								...
								<View style={styles.forgotPasswordContainer}>
									<TouchableOpacity
										...
										<Text style={{ fontWeight: "600", color: Colors.NewTitleColor, ...fontSize(Size.byWidth(16)) }}>Forgot Password?</Text>
									</TouchableOpacity>
								</View>
								
								{Platform.OS == "ios" && (Platform.Version >= 13 || Platform.Version >= "13")&&<TouchableHighlight underlayColor={"#ffffff99"} onPress={this.controller.onClickAppleSignIn.bind(this.controller)}>
										<View style={{marginTop: Size.byWidth(16), width: '100%', 
													  borderWidth: 1, borderColor: "#7c7c7c", height: Size.byHeight(42), 
													  alignItems: "center", justifyContent: "center", flexDirection: "row", 
													  borderRadius: Size.byWidth(5), backgroundColor: "#fff"}}>
										<Image source={apple_icon} style={{tintColor: "#5c5c5c"}}/>
										<Text style={{marginLeft: 10, color: Colors.TextColor, fontWeight : Platform.OS === "ios"? '400':'bold', ...fontSize(20)}}>Sign in with Apple</Text>
										</View>
								</TouchableHighlight>}

								<TouchableHighlight underlayColor={"#ffffff99"} onPress={this.controller.onClickGoogleSignIn.bind(this.controller)}>
										<View style={{marginTop: Size.byWidth(16), width: '100%', 
													  borderWidth: 1, borderColor: "#7c7c7c", height: Size.byHeight(42), 
													  alignItems: "center", justifyContent: "center", flexDirection: "row", 
													  borderRadius: Size.byWidth(5), backgroundColor: "#fff"}}>
										<Image source={google_icon}/>
										<Text style={{marginLeft: 10, color: Colors.TextColor, fontWeight : Platform.OS === "ios"? '400':'bold', ...fontSize(20)}}>Sign in with Google</Text>
										</View>
								</TouchableHighlight>
								{/** Join now UI */}
...
const mapDispatch = (dispatch: Function) => ({
	loginServiceCall: (params: object) => dispatch({ type: LoginServiceStatus.RequestStarted, payload: params }),
	fetchLoginAccounts : (params: object) => dispatch({ type: LoginInstanceStatus.RequestStarted, payload: params }),
