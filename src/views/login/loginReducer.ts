...
function* fetchLoginInstanceRequest(params: any) {
  return loginInstanceRequest(`${kAdmin}`, params)
    .then((response: Response) => response.json())
    .catch((err: any) => err);
}
...
function* loginInstanceService(...action: any[]) {
  try {
    ...
    yield put({
      type: LoginInstanceStatus.RequestSuccess,
      payload: responseBody,
    });
  } catch (err) {
...
function* loginService(...action: any[]) {
  try {
    ...
    if (responseBody.ResponseCode !== 200) {
      yield put({
        type: LoginServiceStatus.RequestFailed,
        payload: responseBody,
      });
      return;
    }
...
export function* loginServiceCall() {
  ...
  yield all([
    takeLatest(LoginServiceStatus.RequestStarted, loginService),
