...
function* getNotification(params: any) {
	delay(500);
	return MemoryService(`https://${Account.selectedData().instanceURL}/api/activities/get_types`, params)
...
function* setFlagNotification(params: any) {
	delay(500);
	return MemoryService(`https://${Account.selectedData().instanceURL}/api/notifications/set_seen_data`, params)
		.then((response: Response) => response.json())
		.catch((err: Error) => Promise.reject(err));
}
...
function* loadMore(params: any) {
	delay(500);
	return MemoryService(`https://${Account.selectedData().instanceURL}/api/activities/get_activity_data`, params)
		.then((response: Response) => response.json())
		.catch((err: Error) => Promise.reject(err));
}
	...
	try {		
		...
		if(responseBody.ResponseCode == "200"){
			responseBody.Details.forEach((element: any, index : any) => {
				element.data = new NotificationDataModel().getNotificationDetails(element.data, false);
...
function* loadMoreNotifications(params : any) {	
	try {		
		...
		if(responseBody.ResponseCode == "200"){			
			let data = new NotificationDataModel().getNotificationDetails(responseBody.Details.data, false);						
			yield put({ type : LoadMore , payload : {index: params.payload.index, data : data}})
		}		
	} catch (err) {
        //console.log(err);
	}
...
export function* watchNotifications() {
	yield all([ takeLatest(GetNotificationAPI, getActivityType),
