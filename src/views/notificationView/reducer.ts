...
export const NotificationsRedux =  (state = initialNotifications, action: Payload) => {
    let newState : any = {...state};
    let key = Account.selectedData().instanceID+"_"+Account.selectedData().userID;

    switch(action.type){       
        case GetActivity : newState.notificationData = action.payload.slice(0);
            break;
        case SeenFlag : newState.notificationData[action.payload.detailsIndex].data[action.payload.dataIndex].unreadFlag = false
            ...
            break;
        case MarkAllRead : newState.notificationData[action.payload.detailsIndex].data.forEach((element : any, index : any) => {
                                newState.notificationData[action.payload.detailsIndex].data[index].unreadFlag = false;
                           });
                           newState.notificationData[action.payload.detailsIndex].unseen_count = 0;
                           Utility.unreadNotification[key] = Utility.unreadNotification[key] - newState.notificationData[action.payload.detailsIndex].unseen_count;
                           ...
                           newState.unreadNot = unreadNotiFlag;                         
            break;         
        case LoadMore : newState.notificationData[action.payload.index].data = newState.notificationData[action.payload.index].data.concat(action.payload.data).slice(0);
            break;
        case AddNewNotification :   let selectedIndex = 0;
                                    ...
                                    notificationList.forEach((element : any, index: any) => {
                                        if(index != 0 && element.nid == details.nid  && 
                                        ...
                                        else if(index == newState.notificationData[selectedIndex].data.length - 1){
                                            newState.notificationData[selectedIndex].unseen_count++;
                                        }
                                    });
                                    if(newState.notificationData[selectedIndex]){
                                    newState.notificationData[selectedIndex].data = notificationList.slice(0);
                                    }
